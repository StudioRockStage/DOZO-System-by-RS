...
const reportFile = path.join(globalDir, `DOZO-DistributionReport-${new Date().toISOString().replace(/[:.]/g,'-')}.json`);
...
try {
  ...
  execSync("npm list -g electron-builder || npm install -g electron-builder", { stdio: "inherit" });
...
try {
  ...
  execSync("npx electron-builder --mac --publish never", { cwd: appDir, stdio: "inherit" });
