...
try {
  ...
  execSync(`git config user.name "${report.configuration.user}"`, {
    stdio: 'inherit',
  });
  execSync(`git config user.email "${report.configuration.email}"`, {
    stdio: 'inherit',
  });
...
try {
  ...
  if (remotes.includes('origin')) {
    ...
    if (currentRemote !== report.configuration.remote) {
      ...
      execSync(`git remote set-url origin ${report.configuration.remote}`, {
        stdio: 'inherit',
      });
    ...
    execSync(`git remote add origin ${report.configuration.remote}`, {
      stdio: 'inherit',
    });
...
if (fs.existsSync(tokenPath)) {
  ...
  try {
    ...
    if (ghToken && ghToken.startsWith('gh')) {
      ...
      try {
        execSync(
          `security add-generic-password -a "${report.configuration.user}" -s dozo_github_token -w "${ghToken}" -U`,
          {
            stdio: 'pipe',
          }
        );
...
console.log(
  `   Checks completados: ${report.summary.checksCompleted}/${report.summary.totalChecks}`
);
