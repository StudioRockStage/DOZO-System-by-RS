...
if (fs.existsSync(gitPath)) {
  ...
  console.log(
    "   ⚠️  No hay repositorio Git (se inicializará al ejecutar dozo-phase-7.js)",
  );
}
console.log();
...
if (fs.existsSync(configPath)) {
  ...
  console.log(
    "   ⚠️  No hay configuración (se creará al ejecutar dozo-phase-7.js)",
  );
}
console.log();
...
const reportPattern = path.resolve(
  __dirname,
  "..",
  "DozoCoreResport/reporte-fase-7-*.json",
);
try {
  const reports = execSync(`ls ${reportPattern} 2>/dev/null || echo ""`, {
    encoding: "utf-8",
  }).trim();
  if (reports) {
