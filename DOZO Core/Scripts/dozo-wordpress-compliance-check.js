...
if (!fs.existsSync(latestBuild)) {
  report.errors.push(
    "No se encontró el archivo ZIP del plugin en Latest Builds.",
  );
...
const hasIncludes = entries.some((e) =>
  e.startsWith("warranty-system-rs/includes/"),
);
const hasPublic = entries.some((e) =>
  e.startsWith("warranty-system-rs/public/"),
);

if (!mainFile)
  report.errors.push("Falta el archivo principal warranty-system-rs.php");
if (!hasAdmin || !hasIncludes || !hasPublic)
  report.warnings.push("Estructura incompleta: admin/includes/public");
...
if (!headers.PluginName || !headers.Version)
  report.errors.push("Faltan cabeceras esenciales Plugin Name/Version.");
if (!headers.UpdateURI)
  report.warnings.push("Falta Update URI para actualizaciones automáticas.");
...
if (hookCount < 5)
  report.warnings.push("Pocos hooks detectados, revisar inicialización.");
...
try {
  ...
  if (issues.length > 0) {
    report.warnings.push(
      `Posibles problemas de sintaxis: ${issues.join(", ")}`,
    );
  }
