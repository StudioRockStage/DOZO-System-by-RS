...
const zipPath = path.join(baseDir, 'Latest Builds', 'Warranty System RS', 'warranty-system-rs.zip');
const globalDir = path.join(baseDir, 'to chat gpt', 'Global');
const reportsDir = path.join(baseDir, 'Archive', 'SessionLogs', new Date().toISOString().replace(/[:.]/g,'-'));
...
if (!entries.some(e => e.entryName.endsWith('warranty-system-rs.php'))) {
  throw new Error('El ZIP no contiene el archivo principal warranty-system-rs.php');
}
...
execSync(`docker cp "${zipPath}" ${container}:/var/www/html/wp-content/plugins/`, { stdio: 'inherit' });

// ---------------------------------------------------------
// 3️⃣ Descomprimir dentro del contenedor
// ---------------------------------------------------------
console.log('🧩 Descomprimiendo plugin dentro del contenedor...');
execSync(`docker exec ${container} bash -c "cd /var/www/html/wp-content/plugins && unzip -o warranty-system-rs.zip && rm warranty-system-rs.zip"`, { stdio: 'inherit' });
...
for (const f of requiredFiles) {
  try {
    execSync(`docker exec ${container} test -f /var/www/html/wp-content/plugins/warranty-system-rs/${f}`);
  } catch {
...
try {
  execSync(`docker exec ${container} wp plugin activate warranty-system-rs --allow-root`, { stdio: 'inherit' });
...
const jsonPath = path.join(globalDir, 'DOZO-Local-Plugin-Deployment-Report.json');
