...
const srcDirOriginal = path.resolve(
  HOME,
  "Documents/Respaldo WS/warranty system",
);
...
if (!fs.existsSync(srcDirOriginal))
  throw new Error("No se encontró la carpeta fuente en Respaldo WS.");
...
if (!mainFile)
  throw new Error("No se encontró el archivo principal del plugin.");
...
if (mainFile !== "warranty-system-rs.php") {
  logStep(
    `Renombrando archivo principal: ${mainFile} → warranty-system-rs.php`,
  );
  fs.renameSync(
  ...
  report.actions.push({
    renamed_main_file: { from: mainFile, to: "warranty-system-rs.php" },
  });
}
