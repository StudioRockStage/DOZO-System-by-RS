...
const srcDir = path.join(baseDir, 'Workspace_TMP_v1.0.1_Wrapper/warranty-system-rs');
...
const requiredDirs = ['admin', 'public', 'assets', 'includes', 'templates', 'tools'];
const missing = [];
...
execSync(`cd "${parentDir}" && zip -r "${zipPath}" "${folderName}" -x "${folderName}/.*" -q`, { stdio: 'inherit' });
...
console.log(`   Tamaño: ${Math.round(stats.size / 1024)} KB (${stats.size} bytes)`);
...
const hasAdmin = entries.some(e => e.entryName.includes('warranty-system-rs/admin/'));
const hasPublic = entries.some(e => e.entryName.includes('warranty-system-rs/public/'));

if (hasAdmin && hasPublic) {
  console.log('   ✓ Directorios admin/ y public/ presentes');
} else {
  console.warn('   ⚠️ Faltan directorios:', !hasAdmin && 'admin/', !hasPublic && 'public/');
}
