...
try {
  ...
  try {
    ...
    const currentEmail = execSync("git config --global user.email", { encoding: 'utf8' }).trim();
...
console.log(`   🔗 GitHub Desktop: ${identity.github_desktop.detected ? 'Detectado' : 'No detectado'}`);
...
console.log('\n💡 NOTA IMPORTANTE:');
console.log('   La configuración Git global NO fue modificada (por seguridad).');
console.log('   Si deseas usar esta identidad para commits Git:');
console.log(`   1. Configura manualmente: git config --global user.name "${identity.name}"`);
