...
if (!fs.existsSync(path.join(srcDir, "warranty-system-rs.php")))
  throw new Error("Falta el archivo principal warranty-system-rs.php.");
...
if (fs.existsSync(toChatGPT)) {
  ...
  for (const n of items)
    fs.renameSync(
      ...
      path.join(trashDir, `${Date.now()}_${n}`),
    );
}
