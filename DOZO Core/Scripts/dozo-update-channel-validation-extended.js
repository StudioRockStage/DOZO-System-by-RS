...
try {
  ...
  logStep('🌐 Validación de conexión FTP', 'OK', { connected: true, host: ftpConfig.host });
  ...
  if (updateJson) {
    ...
    if (missingFields.length > 0) {
      ...
      logStep('🧾 Validación update.json', 'ERROR', { missing_fields: missingFields });
    } else {
      ...
      logStep('🧾 Validación update.json', versionMatch && urlMatch ? 'OK' : 'WARNING', {
        content: jsonContent,
      ...
      if (!versionMatch) {
        report.warnings.push(`Versión en update.json (${jsonContent.version}) diferente de esperada (${expectedVersion})`);
      }
      if (!urlMatch) {
  ...
  if (zipFile) {
    ...
    try {
      ...
      logStep('📦 Estructura ZIP validada', hasRootFolder && hasMainFile ? 'OK' : 'ERROR', {
...
console.log(`✓ Validaciones: ${report.summary.passed}/${report.summary.total_validations}`);
