...
const reportPath = path.join(
  globalDir,
  `DOZO-Rollback-Reinstall-Report-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
);
...
step("Verificando estado de Docker", () => {
  ...
  try {
    execSync(`docker ps -a --filter name=${container} --format "{{.Names}}"`, {
      stdio: "pipe",
    });
    console.log(`  → Contenedor ${container} detectado`);
  } catch (e) {
    report.warnings.push(
      "Contenedor WordPress no encontrado. Ejecutar: ./start-wordpress.sh",
    );
    throw new Error(
      "Contenedor WordPress no encontrado. Ejecutar primero: ./start-wordpress.sh",
    );
  }
});
...
step("Desactivando y eliminando plugin del contenedor", () => {
  ...
  try {
    execSync(
      `docker exec ${container} bash -c "if command -v wp > /dev/null 2>&1; then wp plugin deactivate warranty-system-rs --allow-root 2>/dev/null || true; fi"`,
      { stdio: "pipe" },
    );
  ...
  execSync(
    `docker exec ${container} bash -c "rm -rf /var/www/html/wp-content/plugins/warranty-system-rs"`,
    { stdio: "pipe" },
  );
...
step("Limpiando base de datos y cachés", () => {
  try {
    execSync(
      `docker exec ${container} bash -c "if command -v wp > /dev/null 2>&1; then wp transient delete --all --allow-root 2>/dev/null; wp cache flush --allow-root 2>/dev/null; fi"`,
      { stdio: "pipe" },
    );
...
step("Verificando WP-CLI y unzip en contenedor", () => {
  // Verificar WP-CLI
  try {
    execSync(`docker exec ${container} bash -c "command -v wp"`, {
      stdio: "pipe",
    });
  ...
  // Verificar unzip
  try {
    execSync(`docker exec ${container} bash -c "command -v unzip"`, {
      stdio: "pipe",
    });
    ...
    execSync(
      `docker exec ${container} bash -c "apt update && apt install -y unzip"`,
      { stdio: "inherit" },
    );
  }
});
...
step("Reinstalando plugin base desde ZIP", () => {
  ...
  execSync(`docker cp "${zipPath}" ${container}:/tmp/warranty-system-rs.zip`, {
    stdio: "pipe",
  });
  ...
  execSync(
    `docker exec ${container} bash -c "cd /var/www/html/wp-content/plugins && unzip -o /tmp/warranty-system-rs.zip && rm /tmp/warranty-system-rs.zip"`,
    { stdio: "pipe" },
  );
  ...
  try {
    execSync(
      `docker exec ${container} bash -c "wp plugin activate warranty-system-rs --allow-root"`,
      { stdio: "pipe" },
    );
    console.log("  → Plugin activado exitosamente");
  } catch (e) {
    report.warnings.push(
      "No se pudo activar con WP-CLI. Activar manualmente desde WordPress Admin.",
    );
...
step("Validando reinstalación y estructura", () => {
  ...
  for (const file of requiredFiles) {
    try {
      execSync(
        `docker exec ${container} test -f /var/www/html/wp-content/plugins/warranty-system-rs/${file}`,
        { stdio: "pipe" },
      );
      console.log(`  ✓ ${file}`);
    } catch {
  ...
  for (const dir of requiredDirs) {
    try {
      execSync(
        `docker exec ${container} test -d /var/www/html/wp-content/plugins/warranty-system-rs/${dir}`,
        { stdio: "pipe" },
      );
      console.log(`  ✓ ${dir}/`);
    } catch {
...
report.status =
  report.errors.length === 0 ? "SUCCESS" : "COMPLETED_WITH_ERRORS";
