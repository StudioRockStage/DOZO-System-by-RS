...
// ---------- UTILS ----------
const log = (...a) => console.log('[DOZO-WATCH]', new Date().toLocaleTimeString(), ...a);
...
async function validateZip(zipPath) {
  ...
  if (runNode(VALIDATORS.wpCompliance, 'WordPress Compliance Check')) {
    ...
    results.validations.wp_compliance = fs.existsSync(VALIDATORS.wpCompliance) ? 'FAILED' : 'SKIPPED';
  ...
  if (runNode(VALIDATORS.channelCheck, 'Update Channel Extended Validation')) {
    ...
    results.validations.channel_extended = fs.existsSync(VALIDATORS.channelCheck) ? 'FAILED' : 'SKIPPED';
  ...
  if (runNode(VALIDATORS.channelRecheck, 'Update Channel Final Recheck')) {
    ...
    results.validations.channel_recheck = fs.existsSync(VALIDATORS.channelRecheck) ? 'FAILED' : 'SKIPPED';
...
function startWatcher() {
  ...
  const existing = fs.readdirSync(LATEST).filter(n => n.endsWith('.zip') && isZipOkName(n));
