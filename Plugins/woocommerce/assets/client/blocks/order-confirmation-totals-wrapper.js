(() => {
  "use strict";
  var e,
    o,
    t,
    r = {
      1025: (e, o, t) => {
        const r = window.wp.blocks,
          n = window.wp.blockEditor;
        var i = t(4530),
          l = t(5573),
          a = t(790);
        const s = (0, a.jsxs)(l.SVG, {
            xmlns: "http://www.w3.org/2000/SVG",
            viewBox: "0 0 24 24",
            fill: "none",
            children: [
              (0, a.jsx)("path", {
                stroke: "currentColor",
                strokeWidth: "1.5",
                fill: "none",
                d: "M6 3.75h12c.69 0 1.25.56 1.25 1.25v14c0 .69-.56 1.25-1.25 1.25H6c-.69 0-1.25-.56-1.25-1.25V5c0-.69.56-1.25 1.25-1.25z",
              }),
              (0, a.jsx)("path", {
                fill: "currentColor",
                fillRule: "evenodd",
                d: "M6.9 7.5A1.1 1.1 0 018 6.4h8a1.1 1.1 0 011.1 1.1v2a1.1 1.1 0 01-1.1 1.1H8a1.1 1.1 0 01-1.1-1.1v-2zm1.2.1v1.8h7.8V7.6H8.1z",
                clipRule: "evenodd",
              }),
              (0, a.jsx)("path", {
                fill: "currentColor",
                d: "M8.5 12h1v1h-1v-1zM8.5 14h1v1h-1v-1zM8.5 16h1v1h-1v-1zM11.5 12h1v1h-1v-1zM11.5 14h1v1h-1v-1zM11.5 16h1v1h-1v-1zM14.5 12h1v1h-1v-1zM14.5 14h1v1h-1v-1zM14.5 16h1v1h-1v-1z",
              }),
            ],
          }),
          c = JSON.parse(
            '{"name":"woocommerce/order-confirmation-totals-wrapper","version":"1.0.0","title":"Order Totals Section","description":"Display the order details section.","category":"woocommerce","keywords":["WooCommerce"],"attributes":{"heading":{"type":"string"}},"supports":{"multiple":false,"align":["wide","full"],"html":false,"spacing":{"padding":true,"margin":true,"__experimentalDefaultControls":{"margin":false,"padding":false}}},"textdomain":"woocommerce","apiVersion":3,"$schema":"https://schemas.wp.org/trunk/block.json"}',
          ),
          p = {
            heading: {
              type: "string",
              default: (0, t(7723).__)("Order details", "woocommerce"),
            },
          };
        (0, r.registerBlockType)(c, {
          icon: {
            src: (0, a.jsx)(i.A, {
              icon: s,
              className: "wc-block-editor-components-block-icon",
            }),
          },
          edit: ({ attributes: e, setAttributes: o }) => {
            const t = (0, n.useBlockProps)();
            return (0, a.jsx)("div", {
              ...t,
              children: (0, a.jsx)(n.InnerBlocks, {
                allowedBlocks: ["core/heading"],
                template: [
                  [
                    "core/heading",
                    {
                      level: 2,
                      style: { typography: { fontSize: "24px" } },
                      content: e.heading || "",
                      onChangeContent: (e) => o({ heading: e }),
                    },
                  ],
                  [
                    "woocommerce/order-confirmation-totals",
                    { lock: { remove: !0 } },
                  ],
                ],
              }),
            });
          },
          save: () => (0, a.jsx)(n.InnerBlocks.Content, {}),
          attributes: p,
        });
      },
      790: (e) => {
        e.exports = window.ReactJSXRuntime;
      },
      6087: (e) => {
        e.exports = window.wp.element;
      },
      7723: (e) => {
        e.exports = window.wp.i18n;
      },
      5573: (e) => {
        e.exports = window.wp.primitives;
      },
    },
    n = {};
  function i(e) {
    var o = n[e];
    if (void 0 !== o) return o.exports;
    var t = (n[e] = { exports: {} });
    return (r[e](t, t.exports, i), t.exports);
  }
  ((i.m = r),
    (e = []),
    (i.O = (o, t, r, n) => {
      if (!t) {
        var l = 1 / 0;
        for (p = 0; p < e.length; p++) {
          for (var [t, r, n] = e[p], a = !0, s = 0; s < t.length; s++)
            (!1 & n || l >= n) && Object.keys(i.O).every((e) => i.O[e](t[s]))
              ? t.splice(s--, 1)
              : ((a = !1), n < l && (l = n));
          if (a) {
            e.splice(p--, 1);
            var c = r();
            void 0 !== c && (o = c);
          }
        }
        return o;
      }
      n = n || 0;
      for (var p = e.length; p > 0 && e[p - 1][2] > n; p--) e[p] = e[p - 1];
      e[p] = [t, r, n];
    }),
    (i.n = (e) => {
      var o = e && e.__esModule ? () => e.default : () => e;
      return (i.d(o, { a: o }), o);
    }),
    (t = Object.getPrototypeOf
      ? (e) => Object.getPrototypeOf(e)
      : (e) => e.__proto__),
    (i.t = function (e, r) {
      if ((1 & r && (e = this(e)), 8 & r)) return e;
      if ("object" == typeof e && e) {
        if (4 & r && e.__esModule) return e;
        if (16 & r && "function" == typeof e.then) return e;
      }
      var n = Object.create(null);
      i.r(n);
      var l = {};
      o = o || [null, t({}), t([]), t(t)];
      for (var a = 2 & r && e; "object" == typeof a && !~o.indexOf(a); a = t(a))
        Object.getOwnPropertyNames(a).forEach((o) => (l[o] = () => e[o]));
      return ((l.default = () => e), i.d(n, l), n);
    }),
    (i.d = (e, o) => {
      for (var t in o)
        i.o(o, t) &&
          !i.o(e, t) &&
          Object.defineProperty(e, t, { enumerable: !0, get: o[t] });
    }),
    (i.o = (e, o) => Object.prototype.hasOwnProperty.call(e, o)),
    (i.r = (e) => {
      ("undefined" != typeof Symbol &&
        Symbol.toStringTag &&
        Object.defineProperty(e, Symbol.toStringTag, { value: "Module" }),
        Object.defineProperty(e, "__esModule", { value: !0 }));
    }),
    (i.j = 2085),
    (() => {
      var e = { 2085: 0 };
      i.O.j = (o) => 0 === e[o];
      var o = (o, t) => {
          var r,
            n,
            [l, a, s] = t,
            c = 0;
          if (l.some((o) => 0 !== e[o])) {
            for (r in a) i.o(a, r) && (i.m[r] = a[r]);
            if (s) var p = s(i);
          }
          for (o && o(t); c < l.length; c++)
            ((n = l[c]), i.o(e, n) && e[n] && e[n][0](), (e[n] = 0));
          return i.O(p);
        },
        t = (globalThis.webpackChunkwebpackWcBlocksMainJsonp =
          globalThis.webpackChunkwebpackWcBlocksMainJsonp || []);
      (t.forEach(o.bind(null, 0)), (t.push = o.bind(null, t.push.bind(t))));
    })());
  var l = i.O(void 0, [94], () => i(1025));
  ((l = i.O(l)),
    (((this.wc = this.wc || {}).blocks = this.wc.blocks || {})[
      "order-confirmation-totals-wrapper"
    ] = l));
})();
