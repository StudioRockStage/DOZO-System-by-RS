(() => {
  var e,
    o,
    t,
    r = {
      717: (e, o, t) => {
        "use strict";
        const r = window.wp.blocks;
        var l = t(4530),
          n = t(5573),
          c = t(790);
        const i = (0, c.jsxs)(n.SVG, {
            xmlns: "http://www.w3.org/2000/svg",
            viewBox: "0 0 24 24",
            children: [
              (0, c.jsx)("path", { fill: "none", d: "M0 0h24v24H0V0z" }),
              (0, c.jsx)("path", {
                d: "M15.55 13c.75 0 1.41-.41 1.75-1.03l3.58-6.49c.37-.66-.11-1.48-.87-1.48H5.21l-.94-2H1v2h2l3.6 7.59-1.35 2.44C4.52 15.37 5.48 17 7 17h12v-2H7l1.1-2h7.45zM6.16 6h12.15l-2.76 5H8.53L6.16 6zM7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zm10 0c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2z",
              }),
            ],
          }),
          s = JSON.parse(
            '{"name":"woocommerce/cart-link","title":"Cart Link","icon":"cart","description":"Display a link to the cart.","category":"woocommerce","keywords":["WooCommerce"],"textdomain":"woocommerce","supports":{"interactivity":{"clientNavigation":true},"html":false,"multiple":false,"typography":{"fontSize":true},"color":{"text":false,"link":true},"spacing":{"padding":true}},"example":{"attributes":{"isPreview":true,"cartIcon":"cart","content":"Cart"}},"attributes":{"isPreview":{"type":"boolean","default":false},"cartIcon":{"type":"string","default":"cart"},"content":{"type":"string","default":null}},"apiVersion":3,"$schema":"https://schemas.wp.org/trunk/block.json"}',
          ),
          a = window.wp.blockEditor,
          u = (0, c.jsxs)(n.SVG, {
            xmlns: "http://www.w3.org/2000/svg",
            viewBox: "0 0 32 32",
            fill: "none",
            children: [
              (0, c.jsx)("circle", {
                cx: "12.6667",
                cy: "24.6667",
                r: "2",
                fill: "currentColor",
              }),
              (0, c.jsx)("circle", {
                cx: "23.3333",
                cy: "24.6667",
                r: "2",
                fill: "currentColor",
              }),
              (0, c.jsx)("path", {
                fillRule: "evenodd",
                clipRule: "evenodd",
                d: "M9.28491 10.0356C9.47481 9.80216 9.75971 9.66667 10.0606 9.66667H25.3333C25.6232 9.66667 25.8989 9.79247 26.0888 10.0115C26.2787 10.2305 26.3643 10.5211 26.3233 10.8081L24.99 20.1414C24.9196 20.6341 24.4977 21 24 21H12C11.5261 21 11.1173 20.6674 11.0209 20.2034L9.08153 10.8701C9.02031 10.5755 9.09501 10.269 9.28491 10.0356ZM11.2898 11.6667L12.8136 19H23.1327L24.1803 11.6667H11.2898Z",
                fill: "currentColor",
              }),
              (0, c.jsx)("path", {
                fillRule: "evenodd",
                clipRule: "evenodd",
                d: "M5.66669 6.66667C5.66669 6.11438 6.1144 5.66667 6.66669 5.66667H9.33335C9.81664 5.66667 10.2308 6.01229 10.3172 6.48778L11.0445 10.4878C11.1433 11.0312 10.7829 11.5517 10.2395 11.6505C9.69614 11.7493 9.17555 11.3889 9.07676 10.8456L8.49878 7.66667H6.66669C6.1144 7.66667 5.66669 7.21895 5.66669 6.66667Z",
                fill: "currentColor",
              }),
            ],
          }),
          p = (0, c.jsxs)(n.SVG, {
            xmlns: "http://www.w3.org/2000/svg",
            viewBox: "0 0 32 32",
            fill: "none",
            children: [
              (0, c.jsx)("path", {
                fillRule: "evenodd",
                clipRule: "evenodd",
                d: "M12.4444 14.2222C12.9354 14.2222 13.3333 14.6202 13.3333 15.1111C13.3333 15.8183 13.6143 16.4966 14.1144 16.9967C14.6145 17.4968 15.2927 17.7778 16 17.7778C16.7072 17.7778 17.3855 17.4968 17.8856 16.9967C18.3857 16.4966 18.6667 15.8183 18.6667 15.1111C18.6667 14.6202 19.0646 14.2222 19.5555 14.2222C20.0465 14.2222 20.4444 14.6202 20.4444 15.1111C20.4444 16.2898 19.9762 17.4203 19.1427 18.2538C18.3092 19.0873 17.1787 19.5555 16 19.5555C14.8212 19.5555 13.6908 19.0873 12.8573 18.2538C12.0238 17.4203 11.5555 16.2898 11.5555 15.1111C11.5555 14.6202 11.9535 14.2222 12.4444 14.2222Z",
                fill: "currentColor",
              }),
              (0, c.jsx)("path", {
                fillRule: "evenodd",
                clipRule: "evenodd",
                d: "M11.2408 6.68254C11.4307 6.46089 11.7081 6.33333 12 6.33333H20C20.2919 6.33333 20.5693 6.46089 20.7593 6.68254L24.7593 11.3492C25.0134 11.6457 25.0717 12.0631 24.9085 12.4179C24.7453 12.7727 24.3905 13 24 13H8.00001C7.60948 13 7.25469 12.7727 7.0915 12.4179C6.92832 12.0631 6.9866 11.6457 7.24076 11.3492L11.2408 6.68254ZM12.4599 8.33333L10.1742 11H21.8258L19.5401 8.33333H12.4599Z",
                fill: "currentColor",
              }),
              (0, c.jsx)("path", {
                fillRule: "evenodd",
                clipRule: "evenodd",
                d: "M7 12C7 11.4477 7.44772 11 8 11H24C24.5523 11 25 11.4477 25 12V25.3333C25 25.8856 24.5523 26.3333 24 26.3333H8C7.44772 26.3333 7 25.8856 7 25.3333V12ZM9 13V24.3333H23V13H9Z",
                fill: "currentColor",
              }),
            ],
          }),
          d = (0, c.jsxs)(n.SVG, {
            xmlns: "http://www.w3.org/2000/svg",
            viewBox: "0 0 32 32",
            fill: "none",
            children: [
              (0, c.jsx)("path", {
                fillRule: "evenodd",
                clipRule: "evenodd",
                d: "M19.5556 12.3333C19.0646 12.3333 18.6667 11.9354 18.6667 11.4444C18.6667 10.7372 18.3857 8.05893 17.8856 7.55883C17.3855 7.05873 16.7073 6.77778 16 6.77778C15.2928 6.77778 14.6145 7.05873 14.1144 7.55883C13.6143 8.05893 13.3333 10.7372 13.3333 11.4444C13.3333 11.9354 12.9354 12.3333 12.4445 12.3333C11.9535 12.3333 11.5556 11.9354 11.5556 11.4444C11.5556 10.2657 12.0238 7.13524 12.8573 6.30175C13.6908 5.46825 14.8213 5 16 5C17.1788 5 18.3092 5.46825 19.1427 6.30175C19.9762 7.13524 20.4445 10.2657 20.4445 11.4444C20.4445 11.9354 20.0465 12.3333 19.5556 12.3333Z",
                fill: "currentColor",
              }),
              (0, c.jsx)("path", {
                fillRule: "evenodd",
                clipRule: "evenodd",
                d: "M7.5 12C7.5 11.4477 7.94772 11 8.5 11H23.5C24.0523 11 24.5 11.4477 24.5 12V25.3333C24.5 25.8856 24.0523 26.3333 23.5 26.3333H8.5C7.94772 26.3333 7.5 25.8856 7.5 25.3333V12ZM9.5 13V24.3333H22.5V13H9.5Z",
                fill: "currentColor",
              }),
            ],
          });
        var C = t(7723);
        const w = window.wp.components;
        (t(9261), t(7905), t(1366));
        const f = ({
          count: e = 0,
          icon: o,
          iconColor: t,
          productCountColor: r,
          productCountVisibility: n,
        }) => {
          const i = "always" === n || ("greater_than_zero" === n && e > 0),
            s = i ? e : "";
          return (0, c.jsxs)("span", {
            className: "wc-block-mini-cart__quantity-badge",
            children: [
              (0, c.jsx)(l.A, {
                className: "wc-block-mini-cart__icon",
                color: t?.color,
                size: 20,
                icon: (function (e) {
                  switch (e) {
                    case "cart":
                    default:
                      return u;
                    case "bag":
                      return p;
                    case "bag-alt":
                      return d;
                  }
                })(o),
              }),
              i &&
                (0, c.jsx)("span", {
                  className: "wc-block-mini-cart__badge",
                  style: { background: r?.color },
                  children: s,
                }),
            ],
          });
        };
        (0, r.registerBlockType)(s, {
          icon: {
            src: (0, c.jsx)(l.A, {
              icon: i,
              className: "wc-block-editor-components-block-icon",
            }),
          },
          attributes: { ...s.attributes },
          edit: ({ attributes: e, setAttributes: o }) => {
            const { cartIcon: t, content: r } = e,
              l = (0, a.useBlockProps)();
            return (0, c.jsxs)("div", {
              ...l,
              children: [
                (0, c.jsx)(a.InspectorControls, {
                  children: (0, c.jsx)(w.PanelBody, {
                    title: (0, C.__)("Settings", "woocommerce"),
                    children: (0, c.jsxs)(w.__experimentalToggleGroupControl, {
                      className: "wc-block-editor-mini-cart__cart-icon-toggle",
                      isBlock: !0,
                      label: (0, C.__)("Cart Icon", "woocommerce"),
                      value: t,
                      onChange: (e) => {
                        o({ cartIcon: e });
                      },
                      children: [
                        (0, c.jsx)(w.__experimentalToggleGroupControlOption, {
                          value: "cart",
                          label: (0, c.jsx)(w.Icon, { size: 32, icon: u }),
                        }),
                        (0, c.jsx)(w.__experimentalToggleGroupControlOption, {
                          value: "bag",
                          label: (0, c.jsx)(w.Icon, { size: 32, icon: p }),
                        }),
                        (0, c.jsx)(w.__experimentalToggleGroupControlOption, {
                          value: "bag-alt",
                          label: (0, c.jsx)(w.Icon, { size: 32, icon: d }),
                        }),
                      ],
                    }),
                  }),
                }),
                (0, c.jsxs)("a", {
                  className: "wc-block-cart-link",
                  href: "#cart-pseudo-link",
                  onClick: (e) => e.preventDefault(),
                  children: [
                    (0, c.jsx)(f, { icon: t, productCountVisibility: "never" }),
                    (0, c.jsx)(a.PlainText, {
                      className: "wc-block-cart-link__text",
                      value: null !== r ? r : (0, C.__)("Cart", "woocommerce"),
                      __experimentalVersion: 2,
                      onChange: (e) => o({ content: e }),
                      style: { backgroundColor: "transparent", resize: "none" },
                    }),
                  ],
                }),
              ],
            });
          },
          save: () => null,
        });
      },
      7905: () => {},
      9261: () => {},
      1366: () => {},
      790: (e) => {
        "use strict";
        e.exports = window.ReactJSXRuntime;
      },
      6087: (e) => {
        "use strict";
        e.exports = window.wp.element;
      },
      7723: (e) => {
        "use strict";
        e.exports = window.wp.i18n;
      },
      5573: (e) => {
        "use strict";
        e.exports = window.wp.primitives;
      },
    },
    l = {};
  function n(e) {
    var o = l[e];
    if (void 0 !== o) return o.exports;
    var t = (l[e] = { exports: {} });
    return (r[e](t, t.exports, n), t.exports);
  }
  ((n.m = r),
    (e = []),
    (n.O = (o, t, r, l) => {
      if (!t) {
        var c = 1 / 0;
        for (u = 0; u < e.length; u++) {
          for (var [t, r, l] = e[u], i = !0, s = 0; s < t.length; s++)
            (!1 & l || c >= l) && Object.keys(n.O).every((e) => n.O[e](t[s]))
              ? t.splice(s--, 1)
              : ((i = !1), l < c && (c = l));
          if (i) {
            e.splice(u--, 1);
            var a = r();
            void 0 !== a && (o = a);
          }
        }
        return o;
      }
      l = l || 0;
      for (var u = e.length; u > 0 && e[u - 1][2] > l; u--) e[u] = e[u - 1];
      e[u] = [t, r, l];
    }),
    (n.n = (e) => {
      var o = e && e.__esModule ? () => e.default : () => e;
      return (n.d(o, { a: o }), o);
    }),
    (t = Object.getPrototypeOf
      ? (e) => Object.getPrototypeOf(e)
      : (e) => e.__proto__),
    (n.t = function (e, r) {
      if ((1 & r && (e = this(e)), 8 & r)) return e;
      if ("object" == typeof e && e) {
        if (4 & r && e.__esModule) return e;
        if (16 & r && "function" == typeof e.then) return e;
      }
      var l = Object.create(null);
      n.r(l);
      var c = {};
      o = o || [null, t({}), t([]), t(t)];
      for (var i = 2 & r && e; "object" == typeof i && !~o.indexOf(i); i = t(i))
        Object.getOwnPropertyNames(i).forEach((o) => (c[o] = () => e[o]));
      return ((c.default = () => e), n.d(l, c), l);
    }),
    (n.d = (e, o) => {
      for (var t in o)
        n.o(o, t) &&
          !n.o(e, t) &&
          Object.defineProperty(e, t, { enumerable: !0, get: o[t] });
    }),
    (n.o = (e, o) => Object.prototype.hasOwnProperty.call(e, o)),
    (n.r = (e) => {
      ("undefined" != typeof Symbol &&
        Symbol.toStringTag &&
        Object.defineProperty(e, Symbol.toStringTag, { value: "Module" }),
        Object.defineProperty(e, "__esModule", { value: !0 }));
    }),
    (n.j = 5306),
    (() => {
      var e = { 5306: 0 };
      n.O.j = (o) => 0 === e[o];
      var o = (o, t) => {
          var r,
            l,
            [c, i, s] = t,
            a = 0;
          if (c.some((o) => 0 !== e[o])) {
            for (r in i) n.o(i, r) && (n.m[r] = i[r]);
            if (s) var u = s(n);
          }
          for (o && o(t); a < c.length; a++)
            ((l = c[a]), n.o(e, l) && e[l] && e[l][0](), (e[l] = 0));
          return n.O(u);
        },
        t = (globalThis.webpackChunkwebpackWcBlocksMainJsonp =
          globalThis.webpackChunkwebpackWcBlocksMainJsonp || []);
      (t.forEach(o.bind(null, 0)), (t.push = o.bind(null, t.push.bind(t))));
    })());
  var c = n.O(void 0, [94], () => n(717));
  ((c = n.O(c)),
    (((this.wc = this.wc || {}).blocks = this.wc.blocks || {})["cart-link"] =
      c));
})();
