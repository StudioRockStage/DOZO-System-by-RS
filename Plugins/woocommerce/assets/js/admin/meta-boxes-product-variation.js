...
jQuery(function ($) {
  ...
  var wc_meta_boxes_product_variations_actions = {
    ...
    create_variations: function (event) {
      ...
      var new_attribute_data = $(".woocommerce_variation_new_attribute_data");
      ...
      var original_data = new_attribute_data.find("input, select, textarea");
      ...
      var data = {
        ...
        security: woocommerce_admin_meta_boxes.add_attributes_and_variations,
      };
        ...
        if (response.error) {
          ...
          $.get(this_page_url, function (response) {
            ...
            $("#variable_product_options_inner").replaceWith(
              $(response).find("#variable_product_options_inner"),
            );
    ...
    maybe_enable_button_to_add_price_to_variations: function () {
      var variation_price = parseInt($(".wc_input_variations_price").val(), 10);
      ...
      $(".woocommerce_variations .woocommerce_variation").each(
        function (index, el) {
          $(".variation_menu_order", el)
            .val(
              parseInt(
                $(el).index(".woocommerce_variations .woocommerce_variation"),
                10,
              ) +
                1 +
                offset,
  ...
  var wc_meta_boxes_product_variations_ajax = {
    ...
    save_on_submit_done: function () {
      ...
      if ("publish" === callerid) {
        postForm
          .append('<input type="hidden" name="publish" value="1" />')
          .trigger("submit");
      } else {
        postForm
          .append('<input type="hidden" name="save-post" value="1" />')
          .trigger("submit");
      }
    },
        ...
        $.post(
          ...
          function (response) {
            ...
            if (count > 0) {
              ...
              $(".generate_variations").text("Regenerate variations");
            } else {
              wc_meta_boxes_product_variations_ajax.unblock();
            }
          },
    ...
    input_changed: function (event) {
      ...
      $("button.cancel-variation-changes, button.save-variation-changes").prop(
        "disabled",
        false,
      );
    ...
    do_variation_action: function () {
      ...
      switch (do_variation_action) {
        ...
        case "variable_sale_price_decrease":
           ...
           */
          if (
            do_variation_action === "variable_regular_price_decrease" ||
    ...
    show_hide_variation_empty_state: function () {
      ...
      if (parseInt(wrapper.attr("data-total")) > 0) {
        $("#variable_product_options_inner").removeClass("no-variations");
        $("#field_to_edit").removeClass("hidden");
      } else {
        $("#variable_product_options_inner").addClass("no-variations");
  ...
  var wc_meta_boxes_product_variations_pagenav = {
    ...
    update_single_quantity: function () {
      wc_meta_boxes_product_variations_pagenav.update_variations_count(1);
    ...
    set_paginav: function (qty) {
      var wrapper = $("#variable_product_options").find(
        ...
        total_pages = Math.ceil(
          new_qty / woocommerce_admin_meta_boxes_variations.variations_per_page,
        ),
        options = "";
