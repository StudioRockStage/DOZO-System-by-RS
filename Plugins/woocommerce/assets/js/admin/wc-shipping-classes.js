    ...
    var $tbody = $(".wc-shipping-class-rows"),
      ...
      ShippingClass = Backbone.Model.extend({
        save: function (changes) {
          $.post(
            ajaxurl +
              (ajaxurl.indexOf("?") > 0 ? "&" : "?") +
              "action=woocommerce_shipping_classes_save_changes",
            {
      ...
      ShippingClassView = Backbone.View.extend({
        ...
        initialize: function () {
          ...
          $(document.body).on(
            "click",
            ".wc-shipping-class-add-new",
            { view: this },
            this.configureNewShippingClass,
          );
          $(document.body).on(
            "wc_backbone_modal_response",
            { view: this },
            this.onConfigureShippingClassSubmitted,
          );
          $(document.body).on(
            "wc_backbone_modal_loaded",
            { view: this },
            this.onLoadBackboneModal,
          );
          $(document.body).on(
        ...
        initRow: function (rowData) {
          ...
          $tr.find("select").each(function () {
            ...
            $(this)
              .find('option[value="' + rowData[attribute] + '"]')
              .prop("selected", true);
          });
          ...
          $tr.find(".edit").hide();
          $tr
            .find(".wc-shipping-class-edit")
            .on("click", { view: this }, this.onEditRow);
          $tr
            .find(".wc-shipping-class-delete")
        ...
        onConfigureShippingClassSubmitted: function (
          event,
          target,
          posted_data,
        ) {
