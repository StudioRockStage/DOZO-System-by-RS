  ...
  var wcSystemStatus = {
    ...
    generateReport: function () {
      ...
      $(".wc_status_table thead, .wc_status_table tbody").each(function () {
        if ($(this).is("thead")) {
          var label =
            $(this).find("th:eq(0)").data("exportLabel") || $(this).text();
          report = report + "\n### " + label.trim() + " ###\n\n";
        } else {
          $("tr", $(this)).each(function () {
            var label =
              $(this).find("td:eq(0)").data("exportLabel") ||
              $(this).find("td:eq(0)").text();
            var the_name = label.trim().replace(/(<([^>]+)>)/gi, ""); // Remove HTML.
    ...
    downloadReport: function () {
      var ssr_text = new Blob([$("#debug-report").find("textarea").val()], {
        type: "text/plain",
      });
