...
jQuery(function ($) {
  ...
  var wc_meta_boxes_coupon_actions = {
    ...
    type_options: function () {
      ...
      if ("percent" === select_val) {
        $("#coupon_amount")
          ...
          .addClass("wc_input_decimal");
      } else {
        $("#coupon_amount")
          .removeClass("wc_input_decimal")
    ...
    generate_coupon_code: function (e) {
      ...
      for (
        var i = 0;
        i < woocommerce_admin_meta_boxes_coupon.char_length;
        i++
      ) {
        $result += woocommerce_admin_meta_boxes_coupon.characters.charAt(
          Math.floor(
            Math.random() *
              woocommerce_admin_meta_boxes_coupon.characters.length,
          ),
        );
      }
      $result =
        woocommerce_admin_meta_boxes_coupon.prefix +
        $result +
        woocommerce_admin_meta_boxes_coupon.suffix;
