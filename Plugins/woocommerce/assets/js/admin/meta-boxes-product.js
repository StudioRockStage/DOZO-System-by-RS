...
jQuery(function ($) {
  ...
  $("#catalog-visibility")
    ...
    .on("click", function () {
      ...
      $("input[name=_visibility][value=" + current_visibility + "]").attr(
        "checked",
        "checked",
      );
  ...
  function date_picker_select(datepicker) {
    var option = $(datepicker).next().is(".hasDatepicker")
        ...
        : "maxDate",
      otherDateField =
        "minDate" === option ? $(datepicker).next() : $(datepicker).prev(),
  ...
  function update_attribute_row_indexes() {
    $(".product_attributes .woocommerce_attribute").each(function (index, el) {
      $(".attribute_position", el).val(
        parseInt($(el).index(".product_attributes .woocommerce_attribute"), 10),
      );
    });
  }
  ...
  async function add_attribute_to_list(globalAttributeId) {
    try {
      ...
      const newAttributeListItemHtml = await get_new_attribute_list_item_html(
  ...
  $(".product_attributes").on("blur", "input.attribute_name", function () {
    ...
    if (text === "") {
      $attribute
        ...
        .text(woocommerce_admin_meta_boxes.i18n_attribute_name_placeholder);
    } else {
  ...
  $(".product_attributes").on(
    ...
    function () {
      ...
      $.get(woocommerce_admin_meta_boxes.ajax_url, data, function (response) {
        if (response.errors) {
          // Error.
          ...
          response.forEach(function (term) {
            const currentItem = $wrapper.find(
              'select.attribute_values option[value="' + term.term_id + '"]',
            );
  ...
  $("#postexcerpt > .postbox-header > .hndle")
    ...
    .tipTip({
      attribute: "data-tip",
      content: woocommerce_admin_meta_boxes.i18n_product_short_description_tip,
      fadeIn: 50,
      fadeOut: 50,
      delay: 200,
      keepAlive: true,
  ...
  if (setProductImageLink) {
    $(tooltipMarkup).insertAfter(setProductImageLink).tipTip(tooltipData);
  }

  const addProductImagesLink = $(".add_product_images > a");

  if (addProductImagesLink) {
    $(tooltipMarkup).insertAfter(addProductImagesLink).tipTip(tooltipData);
  }
});
