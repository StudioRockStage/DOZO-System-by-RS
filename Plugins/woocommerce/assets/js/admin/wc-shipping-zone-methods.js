    ...
    var $table = $(".wc-shipping-zone-methods"),
      ...
      ShippingMethod = Backbone.Model.extend({
        ...
        logChanges: function (changedRows) {
          ...
          _.each(changedRows.methods, function (row, id) {
            ...
            changes.methods[id] = _.extend(
              changes.methods[id] || { instance_id: id },
              row,
            );
          });
        ...
        save: function () {
          ...
          if (
            _.has(changes, "zone_locations") &&
            _.isEmpty(changes.zone_locations)
          ) {
          ...
          $.post(
            ajaxurl +
              (ajaxurl.indexOf("?") > 0 ? "&" : "?") +
              "action=woocommerce_shipping_zone_methods_save_changes",
            {
      ...
      ShippingMethodView = Backbone.View.extend({
        ...
        initialize: function () {
          ...
          $(document.body).on(
            "click",
            ".wc-shipping-zone-method-settings",
            { view: this },
            this.onConfigureShippingMethod,
          );
          $(document.body).on(
            "click",
            ".wc-shipping-zone-add-method",
            { view: this },
            this.onAddShippingMethod,
          );
          $(document.body).on(
            "wc_backbone_modal_response",
            this.onConfigureShippingMethodSubmitted,
          );
          $(document.body).on(
            "wc_region_picker_update",
            this.onUpdateZoneRegionPicker,
          );
          $(document.body).on(
            "wc_backbone_modal_next_response",
            this.onAddShippingMethodSubmitted,
          );
          $(document.body).on(
            "wc_backbone_modal_before_remove",
            this.onCloseConfigureShippingMethod,
          );
          $(document.body).on(
            "wc_backbone_modal_back_response",
            this.onConfigureShippingMethodBack,
          );
          $(document.body).on(
            "click",
            ".wc-shipping-zone-postcodes-toggle",
            this.onTogglePostcodes,
          );
          $(document.body).on(
        ...
        render: function () {
          ...
          // Set name.
          $(".wc-shipping-zone-name").text(
            zone_name ? zone_name : data.strings.default_zone_name,
          );
          ...
          if (_.size(methods)) {
            ...
            $.each(methods, function (id, rowData) {
              if ("yes" === rowData.enabled) {
                ...
                rowData.enabled_icon =
                  '<span class="woocommerce-input-toggle woocommerce-input-toggle--disabled">' +
                  data.strings.no +
                  "</span>";
              }
              ...
              var $tr = view.$el.find(
                'tr[data-id="' + rowData.instance_id + '"]',
              );
              ...
              if (!rowData.has_settings) {
                $tr
                  ...
                  .replaceWith(
                    "<span>" +
                      $tr.find(".wc-shipping-zone-method-title > a").text() +
                      "</span>",
                  );
                var $del = $tr.find(".wc-shipping-zone-method-delete");
                $tr
                  .find(".wc-shipping-zone-method-title .row-actions")
                  .empty()
                  .html($del);
              }
            });
            ...
            this.$el
              .find(".wc-shipping-zone-method-delete")
              .on("click", { view: this }, this.onDeleteRow);
            this.$el
              .find(".wc-shipping-zone-method-enabled a")
              .on("click", { view: this }, this.onToggleEnabled);
          } else {
        ...
        onDeleteRow: function (event) {
          ...
          if (
            window.confirm(data.strings.delete_shipping_method_confirmation)
          ) {
        ...
        onToggleEnabled: function (event) {
          var view = event.data.view,
            ...
            enabled =
              $target.closest("tr").data("enabled") === "yes" ? "no" : "yes",
            changes = {};

          event.preventDefault();
          methods[instance_id].enabled = enabled;
          changes.methods = changes.methods || { methods: {} };
          changes.methods[instance_id] = _.extend(
        ...
        updateModelOnSort: function (event) {
          ...
          _.each(methods, function (method) {
            ...
            var new_position = parseInt(
              $table.find('tr[data-id="' + method.instance_id + '"]').index() +
                1,
              10,
            );
        ...
        onConfigureShippingMethod: function (event) {
          ...
          method.settings_html = shippingMethodView.reformatSettingsHTML(
        ...
        unformatShippingMethodNumericValues: function (data) {
          ...
          Object.keys(data).forEach((key) => {
            if (
              numericValuesFields.includes(key) ||
              key.startsWith(flatRateClassCostIdPrefix)
            ) {
              ...
              try {
                const unformattedValue =
                  window.wc.currency.unformatLocalisedMonetaryValue(
                    config,
                    formattedValue,
                  );
        ...
        onConfigureShippingMethodSubmitted: function (
          event,
          target,
          posted_data,
        ) {
          if ("wc-modal-shipping-method-settings" === target) {
            ...
            $.post(
                ...
                "action=woocommerce_shipping_zone_methods_save_settings",
              {
                ...
                data: shippingMethodView.unformatShippingMethodNumericValues(
                  posted_data,
                ),
              },
              function (response, textStatus) {
                if ("success" === textStatus && response.success) {
                  ...
                  if (_.size(shippingMethodView.model.changes)) {
                    ...
                    shippingMethodView.model.onSaveResponse(
                      response,
                      textStatus,
                    );
                  }
        ...
        showErrors: function (errors) {
          var error_html =
            '<div id="woocommerce_errors" class="error notice is-dismissible">';
        ...
        moveAdvancedCostsHelpTip: function (html) {
          ...
          const advancedCostsHelpTip = htmlContent.find(
            "#wc-shipping-advanced-costs-help-text",
          );
          advancedCostsHelpTip.addClass(
        ...
        addCurrencySymbol: function (html) {
          if (
            !window.wc.wcSettings.CURRENCY ||
            !window.wc.currency.localiseMonetaryValue
          ) {
            return html;
          }
          const htmlContent = $(html);
          ...
          priceInputs.addClass(
            `wc-shipping-currency-position-${symbolPosition}`,
          );
          priceInputs.before(
          ...
          priceInputs.each((i) => {
            ...
            const formattedValue = window.wc.currency.localiseMonetaryValue(
              config,
              value,
            );
        ...
        moveHTMLHelpTips: function (html) {
          ...
          labels.each((i) => {
            ...
            if (helpTipsToRetain.some((tip) => id.includes(tip))) {
              const helpTip = htmlContent.find(
                `label[for=${id}] span.woocommerce-help-tip`,
              );
            ...
            if (id === "woocommerce_free_shipping_ignore_discounts") {
              ...
              if (
                fieldset.length &&
                fieldset.find(".wc-shipping-zone-method-fields-help-text")
                  .length === 0
              ) {
                fieldset.append(
                  `<div class="wc-shipping-zone-method-fields-help-text">${text}</div>`,
                );
              }
            }
        ...
        onAddShippingMethodSubmitted: function (
          event,
          target,
          posted_data,
          closeModal,
          ...
          if ("wc-modal-add-shipping-method" === target) {
            ...
            $.post(
              ajaxurl +
                (ajaxurl.indexOf("?") > 0 ? "&" : "?") +
                "action=woocommerce_shipping_zone_add_method",
              {
                wc_shipping_zones_nonce: data.wc_shipping_zones_nonce,
              ...
              function (response, textStatus) {
                ...
                if (method.settings_html) {
                  method.settings_html =
                    shippingMethodView.reformatSettingsHTML(
                      method.settings_html,
                    );
                  ...
                  shippingMethodView.highlightOnFocus(
        ...
        possiblyHideFreeShippingRequirements: function (data) {
          if (
            Object.keys(data).includes("woocommerce_free_shipping_requires")
          ) {
            const shouldHideRequirements =
              data.woocommerce_free_shipping_requires === null ||
            ...
            allOtherLabelElementsAfter.each((i) => {
              $(allOtherLabelElementsAfter[i]).css(
                "display",
                shouldHideRequirements ? "none" : "block",
              );
            });

            allOtherFieldsetElementsAfter.each((i) => {
              $(allOtherFieldsetElementsAfter[i]).css(
                "display",
                shouldHideRequirements ? "none" : "block",
              );
            });
        ...
        onModalLoaded: function (event, target) {
          if (target === "wc-modal-shipping-method-settings") {
            const select = $("#woocommerce_free_shipping_requires");
            if (select.length > 0) {
              event.data.view.possiblyHideFreeShippingRequirements({
                woocommerce_free_shipping_requires: select.val(),
              });
            }
            ...
            if (
              ...
              $(".wc-shipping-modal-price").on("input", function () {
                ...
                $(this)
                  .siblings("span.wc-shipping-invalid-price-message")
                  .remove();
                ...
                if (!WCNumberValidation.isValidFormattedNumber(value, config)) {
                  ...
                  $(
                    '<span class="wc-shipping-zone-method-fields-help-text wc-shipping-invalid-price-message">' +
                      shippingZoneMethodsLocalizeScript.strings
        ...
        onCloseConfigureShippingMethod: function (
          event,
          target,
          post_data,
          addButtonCalled,
