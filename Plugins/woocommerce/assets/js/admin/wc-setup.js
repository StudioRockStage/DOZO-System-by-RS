...
jQuery(function ($) {
  ...
  $("#store_country").on("change", function () {
    ...
    if (
      $.isEmptyObject(country_postcode_obj) ||
      country_postcode_obj.required
    ) {
    ...
    function () {
      if ($(this).is(":checked")) {
        ...
        $(this)
          ...
          .find(".wc-wizard-service-settings")
          .removeClass("hide");
      } else {
  ...
  $(".wc-wizard-services").on("keyup", function (e) {
    ...
    if (
      $focused.is(".wc-wizard-service-toggle, .wc-wizard-service-enable") &&
      (13 === code || 32 === code)
    ) {
  ...
  $(".wc-wizard-services-list-toggle").on("click", function () {
    ...
    $(this)
      .closest(".wc-wizard-services")
      .find(".wc-wizard-service-item")
      .slideToggle()
      .css("display", "flex");
  });

  $(".wc-wizard-services")
    .on("change", ".wc-wizard-shipping-method-select .method", function (e) {
  ...
  $(".wc-wizard-services").on(
    "change",
    ".wc-wizard-shipping-method-enable",
    function () {
  ...
  function waitForJetpackInstall() {
    wp.ajax
      .post("setup_wizard_check_jetpack")
      .then(function (result) {
        // If we receive success, or an unexpected result
        // let the form submit.
        if (!result || !result.is_active || "yes" === result.is_active) {
  ...
  $(".wc-wizard-services")
    .on(
      "change",
      "input#stripe_create_account, input#ppec_paypal_reroute_requests",
      function () {
        if ($(this).is(":checked")) {
          $(this)
            .closest(".wc-wizard-service-settings")
    ...
    .find("input#stripe_create_account, input#ppec_paypal_reroute_requests")
    .trigger("change");
  ...
  function addPlugins(bySlug, $el, hover) {
    ...
    for (var i in Array.isArray(plugins) ? plugins : []) {
      ...
      bySlug[slug] =
        bySlug[slug] ||
        $('<span class="plugin-install-info-list-item">').append(
