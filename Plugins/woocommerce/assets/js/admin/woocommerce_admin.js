  ...
  $(function () {
    ...
    if (0 === $blankslate.length) {
      if (woocommerce_admin.urls.add_product) {
        $title_action.first().attr("href", woocommerce_admin.urls.add_product);
      }
    ...
    $(document.body)
      ...
      .on(
        ...
        function () {
          ...
          if (
            ...
            regex = new RegExp(
              "[^-0-9%\\" + woocommerce_admin.mon_decimal_point + "]+",
              "gi",
            );
      ...
      .on("keyup", "input[type=text][name*=_global_unique_id]", function () {
      ...
      .on("change", "input[type=text][name*=_global_unique_id]", function () {
        var global_unique_id = $(this).val();
        $(this).val(
          global_unique_id.replace(/[^0-9\-]/g, "").replace(/^-+|-+$/g, ""),
        );
        ...
        if (
          ...
          if (!shifted && !controlled) {
            ...
            if ($("tr.last_selected", $this_table).length > 0) {
              if (
                $this_row.index() > $("tr.last_selected", $this_table).index()
              ) {
                $("tr", $this_table)
                  .slice(
                ...
                $("tr", $this_table)
                  .slice(
                    $this_row.index(),
                    $("tr.last_selected", $this_table).index() + 1,
                  )
                  .addClass("current");
              }
            }
  ...
  $(function () {
    ...
    var wc_order_lock = {
      init: function () {
        // Order screen.
        this.$lock_dialog = $(
          ".woocommerce_page_wc-orders #post-lock-dialog.order-lock-dialog",
        );
        if (
        ...
        // Orders list table.
        this.$list_table = $(
          ".woocommerce_page_wc-orders table.wc-orders-list-table",
        );
        ...
        setTimeout(function () {
          ...
          if (selectedProductIds.length > 0) {
            ...
            url.searchParams.set(
              "_wpnonce",
              woocommerce_admin.nonces.export_selected_products_nonce,
            );
