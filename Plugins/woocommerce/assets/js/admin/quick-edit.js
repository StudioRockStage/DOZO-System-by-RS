...
jQuery(function ($) {
  $("#the-list").on("click", ".editinline", function () {
    ...
    var formatted_regular_price = regular_price.replace(
        ".",
        woocommerce_admin.mon_decimal_point,
      ),
      formatted_sale_price = sale_price.replace(
        ".",
        woocommerce_admin.mon_decimal_point,
      );
    ...
    if (cogs_data.length > 0) {
      var formatted_cogs_value = cogs_data
        .text()
        .replace(".", woocommerce_admin.mon_decimal_point);
      $('input[name="_cogs_value"]', ".inline-edit-row").val(
        formatted_cogs_value,
      );
    }

    $('input[name="_sku"]', ".inline-edit-row").val(sku);
    $('input[name="_regular_price"]', ".inline-edit-row").val(
      formatted_regular_price,
    );
    $('input[name="_sale_price"]', ".inline-edit-row").val(
    ...
    $('select[name="_shipping_class"] option:selected', ".inline-edit-row")
      .attr("selected", false)
      .trigger("change");
    $('select[name="_shipping_class"] option[value="' + shipping_class + '"]')
    ...
    $(
      'select[name="_stock_status"] ~ .wc-quick-edit-warning',
      ".inline-edit-row",
    ).toggle(is_variable_product);
    $(
      'select[name="_stock_status"] option[value="' +
        (is_variable_product ? "" : stock_status) +
        '"]',
      ".inline-edit-row",
    ).attr("selected", "selected");

    $(
      'select[name="_tax_status"] option[value="' + tax_status + '"]',
      ".inline-edit-row",
    ).attr("selected", "selected");
    $(
      'select[name="_tax_class"] option[value="' + tax_class + '"]',
      ".inline-edit-row",
    ).attr("selected", "selected");
    $(
      'select[name="_visibility"] option[value="' + visibility + '"]',
      ".inline-edit-row",
    ).attr("selected", "selected");
    $(
      'select[name="_backorders"] option[value="' + backorders + '"]',
      ".inline-edit-row",
    ).attr("selected", "selected");
    ...
    var product_supports_stock_fields =
      "external" !== product_type && "grouped" !== product_type;

    $(
      ".stock_fields, .manage_stock_field, .stock_status_field, .backorder_field",
    ).show();

    if (product_supports_stock_fields) {
      if ("yes" === manage_stock) {
        $(".stock_qty_field, .backorder_field", ".inline-edit-row")
          .show()
          .removeAttr("style");
    ...
    $('input[name="comment_status"]')
      .parent()
      .find(".checkbox-title")
      .text(woocommerce_quick_edit.strings.allow_reviews);
  });
  ...
  $("#wpbody").on("click", "#doaction, #doaction2", function () {
    ...
    $("#woocommerce-fields-bulk")
      .find(".inline-edit-group .change-input")
      .hide();
  });
