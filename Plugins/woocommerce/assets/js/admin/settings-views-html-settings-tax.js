  ...
  $(function () {
    ...
    var rowTemplate = wp.template("wc-tax-table-row"),
        ...
        save: function () {
          ...
          Backbone.ajax({
            ...
            url:
              ajaxurl +
              (ajaxurl.indexOf("?") > 0 ? "&" : "?") +
              "action=woocommerce_tax_rates_save_changes",
            data: {
      ...
      WCTaxTableViewConstructor = Backbone.View.extend({
        ...
        page: data.page,
        initialize: function () {
          var qty_pages = Math.ceil(
            _.toArray(this.model.get("rates")).length / this.per_page,
          );
        ...
        render: function () {
          ...
          if (paged_rates.length) {
            ...
            this.el.innerHTML = paged_rates
              .map((rowData) => view.rowTemplate(rowData))
              .join("");
          } else {
          ...
          if ($current.length) {
            ...
            reordered_rates = _.map(rates_to_reorder, function (rate) {
              ...
              changes[rate.tax_rate_id] = _.extend(
                changes[rate.tax_rate_id] || {},
                { tax_rate_order: rate.tax_rate_order },
              );
              return rate;
            });
          } else {
            newRow.tax_rate_order =
              1 +
              _.max(_.pluck(rates, "tax_rate_order"), function (val) {
        ...
        onDeleteRow: function (event) {
          ...
          if (($current = $tbody.children(".current"))) {
            $current.each(function () {
              ...
              changes[current_id] = _.extend(changes[current_id] || {}, {
                deleted: "deleted",
              });
            });
        ...
        onPageChange: function (event) {
          ...
          event.data.view.page = $target.data("goto")
            ? $target.data("goto")
            : $target.val();
          event.data.view.render();
          event.data.view.updateUrl();
        ...
        onExport: function (event) {
          var csv_data =
            ...
            data.strings.csv_data_cols.join(",") +
            "\n";

          $.each(
            event.data.view.model.getFilteredRates(),
            function (id, rowData) {
              var row = "";
              ...
              row +=
                (rowData.postcode ? rowData.postcode.join("; ") : "") + ",";
        ...
        updateModelOnChange: function (event) {
          ...
          if (
            "tax_rate_compound" === attribute ||
            "tax_rate_shipping" === attribute
          ) {
