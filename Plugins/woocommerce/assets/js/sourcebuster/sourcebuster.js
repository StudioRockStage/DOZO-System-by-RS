          ...
          var sbjs = {
            init: function (prefs) {
              ...
              if (
                prefs &&
                prefs.callback &&
                typeof prefs.callback === "function"
              ) {
            ...
            pack: {
              main: function (sbjs) {
                return (
                  ...
                  sbjs.id +
                  data.delimiter +
                  data.aliases.main.platform +
                  "=" +
                  sbjs.platform +
                  data.delimiter +
                  data.aliases.main.format +
                  "=" +
                  sbjs.format +
                  data.delimiter +
                  data.aliases.main.tactic +
                  "=" +
                  sbjs.tactic
                );
              },
                ...
                if (c.indexOf(nameEQ) === 0) {
                  ...
                  if (/^[A-Za-z0-9+/]+$/.test(cookie_content)) {
                    try {
                      ...
                      cookie_content = atob(
                        cookie_content.padEnd(
                          Math.ceil(cookie_content.length / 4) * 4,
                          "=",
                        ),
                      );
                    } catch (error) {
            ...
            parse: function (yummy) {
              ...
              for (var i1 = 0; i1 < cookies.length; i1++) {
                ...
                for (var i2 = 0; i2 < cookie_array.length; i2++) {
                  ...
                  data[this.unsbjs(cookies[i1])][result_array[0]] =
                    this.decodeData(result_array[1]);
                }
              }
          ...
          module.exports = function (prefs) {
            ...
            function mainData() {
              ...
              if (
                ...
                !cookies.get(data.containers.session) &&
                checkReferer(terms.traffic.referral)
              ) {
                setFirstAndCurrentExtraData();
                sbjs_data = getData(terms.traffic.referral);
              } else if (
                !cookies.get(data.containers.first) &&
            ...
            function getData(type) {
              switch (type) {
                ...
                case terms.traffic.organic:
                  ...
                  __sbjs_source =
                    __sbjs_source || uri.getHost(document.referrer);
            ...
            function checkReferer(type) {
              ...
              switch (type) {
                case terms.traffic.organic:
                  return (
                    !!referer && checkRefererHost(referer) && isOrganic(referer)
                  );
                case terms.traffic.referral:
                  return (
                    !!referer &&
                    checkRefererHost(referer) &&
                    isReferral(referer)
                  );
                default:
                  return false;
            ...
            function checkRefererHost(referer) {
              if (p.domain) {
                ...
                return (
                  uri.getHost(referer) !== uri.getHost(document.location.href)
                );
              }
            }
            ...
            function isOrganic(referer) {
              ...
              var y_host_regex = new RegExp(
                  "^(?:.*\\.)?" + utils.escapeRegexp(y_host) + "\\..{2,9}$",
                ),
                y_param_regex = new RegExp(
                  ".*" + utils.escapeRegexp(y_param) + "=.*",
                ),
                g_host_regex = new RegExp(
                  "^(?:www\\.)?" + utils.escapeRegexp(g_host) + "\\..{2,9}$",
                );
                  ...
                  if (
                    ...
                    uri
                      ...
                      .query.match(
                        new RegExp(
                          ".*" +
                            utils.escapeRegexp(p.organics[i].param) +
                            "=.*",
                          "i",
                        ),
                      )
              ...
              if (p.referrals.length > 0) {
                for (var i = 0; i < p.referrals.length; i++) {
                  if (
                    ...
                    __sbjs_source =
                      p.referrals[i].display || p.referrals[i].host;
                    __sbjs_medium =
                      p.referrals[i].medium || terms.referer.referral;
                    return true;
                  }
            ...
            (function setData() {
              ...
              if (!cookies.get(data.containers.udata)) {
                ...
                visits =
                  parseInt(
                    cookies.parse(data.containers.udata)[
                      cookies.unsbjs(data.containers.udata)
                    ][data.aliases.udata.visits],
                  ) || 1;
                visits = cookies.get(data.containers.session)
                  ? visits
                  : visits + 1;
                udata = data.pack.user(visits, p.user_ip);
              }
              cookies.set(
                data.containers.udata,
                udata,
                lifetime,
                domain,
                isolate,
              ...
              if (!cookies.get(data.containers.session)) {
                ...
                pages_count =
                  parseInt(
                    cookies.parse(data.containers.session)[
                      cookies.unsbjs(data.containers.session)
                    ][data.aliases.session.pages_seen],
                  ) || 1;
                pages_count += 1;
              }
        ...
        function (_dereq_, module, exports) {
          ...
          module.exports = {
            go: function (lifetime, domain, isolate) {
              ...
              if (
                !cookies.get(data.containers.first) &&
                !cookies.get(data.service.migrations)
              ) {
                var mids = [];
          ...
          module.exports = {
            fetch: function (prefs) {
              ...
              params.session_length =
                this.validate.checkInt(user.session_length) || 30;

              // Set `timezone offset` in hours
              params.timezone_offset = this.validate.checkInt(
            ...
            validate: {
              checkFloat: function (v) {
                return v && this.isNumeric(parseFloat(v))
                  ? parseFloat(v)
                  : false;
              },
