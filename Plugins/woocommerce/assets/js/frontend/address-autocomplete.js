  ...
  (function () {
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      async function displaySuggestions(inputValue, country, type) {
        ...
        try {
          const filteredSuggestions =
            await window.wc.addressAutocomplete.activeProvider[type].search(
              sanitizedInput,
              country,
              type,
            );
          ...
          const safeSuggestions = filteredSuggestions.slice(0, maxSuggestions);
      ...
      async function selectAddress(type, addressId) {
        ...
        try {
          addressData =
            await window.wc.addressAutocomplete.activeProvider[type].select(
              addressId,
            );
        } catch (error) {
          console.error(
        ...
        if (addressData.country) {
          setFieldValue(addressInputs[type]["country"], addressData.country);
        }
        ...
        addressSelectionTimeout = setTimeout(function () {
          ...
          if (addressData.city) {
            setFieldValue(addressInputs[type]["city"], addressData.city);
          } else {
          ...
          if (addressData.state) {
            setFieldValue(addressInputs[type]["state"], addressData.state);
          } else {
      ...
      function setActiveSuggestion(type, index) {
        ...
        if (newActiveLi) {
          ...
          addressInput.setAttribute("aria-activedescendant", newActiveLi.id);
      ...
      addressTypes.forEach((type) => {
        ...
        if (addressInput && countryInput) {
          ...
          addressInput.addEventListener("keydown", async function (e) {
            ...
            if (e.key === "ArrowDown") {
              e.preventDefault();
              newIndex = (activeSuggestionIndices[type] + 1) % items.length;
              setActiveSuggestion(type, newIndex);
            } else if (e.key === "ArrowUp") {
              e.preventDefault();
              newIndex =
                (activeSuggestionIndices[type] - 1 + items.length) %
                items.length;
              setActiveSuggestion(type, newIndex);
            } else if (e.key === "Enter") {
              if (activeSuggestionIndices[type] > -1) {
                e.preventDefault();
                const selectedItem = suggestionsLists[type].querySelector(
                ...
                hideSuggestions(type);
                enableBrowserAutofill(addressInput);
                await selectAddress(type, selectedItem.dataset.id);
      ...
      document.addEventListener("click", function (event) {
        addressTypes.forEach((type) => {
          ...
          if (
            ...
            if (addressInputs[type] && addressInputs[type]["address_1"]) {
              enableBrowserAutofill(addressInputs[type]["address_1"], false);
            }
          }
        });
      });
