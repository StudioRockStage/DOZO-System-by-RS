(function () {
  ...
  document.addEventListener("wp_listen_for_consent_change", (e) => {
    ...
    for (const key in changedConsentCategory) {
      if (
        ...
        key === window.wc_order_attribution.params.consentCategory
      ) {
        window.wc_order_attribution.setOrderTracking(
          changedConsentCategory[key] === "allow",
        );
      }
    }
  });
  ...
  document.addEventListener("wp_consent_type_defined", () => {
    window.wc_order_attribution.setOrderTracking(
      wp_has_consent(window.wc_order_attribution.params.consentCategory),
    );
  });
})();
