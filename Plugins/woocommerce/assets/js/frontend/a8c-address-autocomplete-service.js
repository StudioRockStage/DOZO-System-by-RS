(function () {
  ...
  Object.entries(a8cAddressAutocompleteServiceKeys).forEach(([key, value]) => {
    ...
    const handleApiError = (data, response) => {
      ...
      switch (errorCode) {
        ...
        case "rate_limit_exceeded":
          ...
          setTimeout(
            () => {
              const index = permanentlyDisabledServices.indexOf(key);
              if (index !== -1) {
                permanentlyDisabledServices.splice(index, 1);
              }
            },
            (Number(response.headers.get("RateLimit-Retry-After")) || 60) *
              1000,
          );
          console.error(
        ...
        case "woo_address_suggestion_server_error":
          serviceErrorRetries++;
          if (serviceErrorRetries >= MAX_SERVICE_ERROR_RETRIES) {
            permanentlyDisabledServices.push(key);
            console.error(
    ...
    const debouncedSearch = debounce(
      async (inputValue, country) => {
        ...
        try {
          ...
          console.error(`Error fetching address suggestions for ${key}:`, e);
          return [];
        }
      },
      300,
    ...
    window.wc.addressAutocomplete.registerAddressAutocompleteProvider({
      ...
      async select(addressId) {
        ...
        const response = await fetch(`${selectUrl}?${params.toString()}`);
...
function createBeacon(section, { name, value, type }) {
  ...
  return value.map(
    (v) => `a8c.${section}.${event}:${v}|${type === "timing" ? "ms" : "c"}`,
  );
}
