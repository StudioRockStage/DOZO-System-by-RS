  ...
  $.flexslider = function (el, options) {
    ...
    methods = {
      init: function () {
        ...
        // TOUCH/USECSS:
        slider.transforms = slider.transitions =
          !slider.vars.video && !fade && slider.vars.useCSS;
      ...
      smoothHeight: function (dur) {
        if (!vertical || fade) {
          ...
          $obj.css({
            ...
            transition: dur ? "height " + dur + "ms" : "none",
          });
        }
      },
        ...
        if (!fade) {
          ...
          if (!touch) {
            ...
            slider.slides
              .eq(target)
              .off("transitionend")
              .on("transitionend", slider.wrapup);
          }
      ...
      if (!fade) {
        ...
        if (type === "init") {
          if (!touch) {
            ...
            slider.slides.css({
              transition:
                "opacity " + slider.vars.animationSpeed / 1000 + "s " + easing,
            });
          } else {
            slider.slides
              .css({
