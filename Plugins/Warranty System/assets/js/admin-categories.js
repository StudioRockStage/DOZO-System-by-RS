...
(function ($) {
  ...
  function syncCategories() {
    ...
    $btn
      .prop("disabled", true)
      .html(
        '<i class="rs-icon" data-icon="loader"></i><span>Sincronizando...</span>',
      );
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          rsShowNotification(
            "✅ Categorías sincronizadas: " + response.data.total,
            "success",
          );
          reloadCategoryTable();
          ...
          rsShowNotification(
            "❌ " + (response.data.message || "Error al sincronizar"),
            "error",
          );
        }
      },
      ...
      complete: function () {
        $btn
          .prop("disabled", false)
          .html(
            '<i class="rs-icon" data-icon="refresh-cw"></i><span>Sincronizar con WooCommerce</span>',
          );
      },
    });
  }
  ...
  function saveCategory() {
    ...
    $btn
      .prop("disabled", true)
      .html(
        '<i class="rs-icon" data-icon="loader"></i><span>Guardando...</span>',
      );

    console.log(
      "📡 DOZO v4.0: Iniciando guardado de categoría ID:",
      categoryId,
    );
    ...
    $.ajax({
      ...
      complete: function () {
        $btn
          .prop("disabled", false)
          .html(
            '<i class="rs-icon" data-icon="check"></i><span>Guardar Configuración</span>',
          );
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          $("#inactiveCount, #inactiveCount2").text(
  ...
  function restoreDefaults() {
    if (
      !confirm("¿Restaurar todas las categorías a sus valores predeterminados?")
    ) {
      return;
    }
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          rsShowNotification(
            "❌ " + (response.data.message || "Error al restaurar"),
            "error",
          );
        }
      },
