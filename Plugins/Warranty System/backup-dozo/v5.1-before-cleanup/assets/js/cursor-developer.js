...
(function($) {
    ...
    const CursorDeveloper = {
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        this.addLog('Configuración guardada: ' + $('#cursorEndpointUrl').val(), 'success');
        ...
        executeQuery: function() {
            ...
            this.addLog('Query: ' + query.substring(0, 100) + (query.length > 100 ? '...' : ''), 'info');
        ...
        displayResponse: function(data) {
            ...
            if (response.code) {
                ...
                html += '<button class="rs-btn rs-btn-secondary" onclick="CursorDeveloper.copyCode(this)">📋 Copiar</button>';
                html += '<button class="rs-btn rs-btn-secondary" onclick="CursorDeveloper.previewCode(this)">👁️ Preview</button>';
                html += '<button class="rs-btn rs-btn-primary" onclick="CursorDeveloper.applyCode(this)">✅ Aplicar Fix</button>';
                html += '</div>';
                html += '</div>';
            }

            if (response.explanation) {
                html += '<div style="margin-top: 16px; padding: 16px; background: #f9fafb; border-radius: 8px; line-height: 1.6;">';
        ...
        applyCode: function(btn) {
            ...
            const filePath = prompt('Ingresa la ruta del archivo (ej: assets/js/admin-categories.js):');
            
            if (!filePath) {
                return;
            }

            if (!confirm('¿Estás seguro de aplicar este fix?\n\nSe creará un backup automáticamente.')) {
                return;
            }
        ...
        loadQuickAction: function(action) {
            const prompts = {
                audit: 'Realiza una auditoría completa del código del plugin RockStage Warranty System. Enfócate en:\n- Seguridad (XSS, CSRF, SQL injection)\n- Performance (queries lentas, memory leaks)\n- Best practices de WordPress',
                optimize: 'Analiza y optimiza el archivo assets/js/admin-categories.js:\n- Reduce complejidad ciclomática\n- Mejora manejo de promesas\n- Implementa lazy loading donde sea posible',
                security: 'Verifica la seguridad del plugin:\n- Validación de nonces en todos los endpoints AJAX\n- Sanitización de inputs\n- Escapado de outputs\n- Capability checks',
                debug: 'Analiza los logs recientes y ayúdame a debuggear el siguiente error:\n[Pega aquí el mensaje de error o describe el problema]',
                refactor: 'Refactoriza el siguiente código aplicando SOLID principles y design patterns:\n[Pega aquí el código a refactorizar]',
        ...
        addLog: function(message, level = 'info') {
            ...
            $logEntry.append(`<span class="rs-log-level ${level}">${level.toUpperCase()}</span>`);
            $logEntry.append(`<span class="rs-log-message">${this.escapeHtml(message)}</span>`);
        ...
        processMarkdown: function(text) {
            ...
            // Code blocks
            text = text.replace(/```(\w+)?\n([\s\S]*?)```/g, '<div class="rs-code-block"><pre>$2</pre></div>');
            
            // Inline code
            text = text.replace(/`([^`]+)`/g, '<code style="background: #e5e7eb; padding: 2px 6px; border-radius: 4px; font-family: monospace;">$1</code>');
        ...
        saveQueryHistory: function(query, response) {
            try {
                let history = JSON.parse(localStorage.getItem('rs_cursor_history') || '[]');
        ...
        loadQueryHistory: function() {
            try {
                const history = JSON.parse(localStorage.getItem('rs_cursor_history') || '[]');
                this.addLog(`Historial cargado: ${history.length} queries previos`, 'info');
            } catch (e) {
