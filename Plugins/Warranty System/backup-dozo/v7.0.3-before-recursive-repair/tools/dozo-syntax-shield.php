...
/**
 * DOZO v7.0.3: Validate core files before loading - Enhanced with structure check
 */
function dozo_validate_core_files() {
    ...
    foreach ($critical_files as $file) {
        if (!file_exists($file)) {
            error_log('DOZO v7.0.3: Critical file missing: ' . basename($file));
            $has_errors = true;
            continue;
        }
        ...
        if (!dozo_check_class_integrity($file)) {
            error_log('DOZO v7.0.3: Integrity check failed: ' . basename($file));
            $has_errors = true;
        }
        
        // Step 2: Advanced structure check (v7.0.3)
        if (!dozo_core_structure_check($file)) {
            error_log('DOZO v7.0.3: Structure check failed: ' . basename($file));
            $has_errors = true;
        }
    }
    ...
    error_log('✅ DOZO v7.0.3: All validation checks passed for all core files');
    return true;
}
...
add_action('plugins_loaded', function() {
    if (!defined('DOZO_SAFE_MODE') || DOZO_SAFE_MODE !== true) {
        error_log('✅ DOZO v7.0.3 initialized successfully - Syntax Shield + Structure Check active');
    }
}, 999);
