...
<div class="wrap rs-warranty-settings">
    ...
    <div class="rs-tabs-wrapper">
        ...
        <button class="rs-tab-btn <?php echo $current_tab === 'design' ? 'active' : ''; ?>" onclick="rsWarrantySwitchTab('design')">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" stroke-width="2"/></svg>
            🎨 Diseño
        </button>
    </div>
    ...
    <div id="tab-categories" class="rs-tab-content <?php echo $current_tab === 'categories' ? 'active' : ''; ?>" role="tabpanel" aria-labelledby="tab-btn-categories">
        ...
        <div class="rs-action-bar">
            ...
            <div class="rs-actions rs-actions--gap">
                ...
                <button type="button" class="rs-btn rs-btn--secondary" onclick="location.reload()" data-action="refresh">
                    <i class="rs-icon" data-icon="refresh-cw"></i>
    ...
    <div id="tab-advanced" class="rs-tab-content <?php echo $current_tab === 'advanced' ? 'active' : ''; ?>">
        <form method="post" action="<?php echo admin_url('admin-post.php'); ?>">
            ...
            <div class="rs-settings-card" style="background: linear-gradient(135deg, rgba(255,140,0,0.05), rgba(255,140,0,0.1)); border-color: #FF8C00;">
                ...
                <!-- DOZO v7.0: Visual Health Bar -->
                <div id="dozoHealthBar" style="margin-bottom: 24px; padding: 20px; background: linear-gradient(135deg, rgba(255,140,0,0.05), rgba(255,140,0,0.1)); border: 1px solid rgba(255,140,0,0.2); border-radius: 12px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                        <span style="font-weight: 600; color: #1f2937; font-size: 14px;">🏥 Estado de Salud del Sistema</span>
                        <span id="dozoHealthScore" style="font-weight: 700; color: #FF8C00; font-size: 18px;">--</span>
                    </div>
                    <div style="width: 100%; height: 8px; background: #e5e7eb; border-radius: 4px; overflow: hidden;">
                        <div id="dozoHealthProgress" style="width: 0%; height: 100%; background: linear-gradient(90deg, #10b981, #22c55e); transition: width 1s ease, background 0.3s ease;"></div>
                    </div>
                    <div id="dozoHealthStatus" style="margin-top: 12px; font-size: 13px; color: #6b7280; display: none;">
                        <span id="dozoHealthMessage">Calculando estado de salud...</span>
                    </div>
                </div>
    ...
    <!-- Tab: Design - DOZO v7.2 -->
    <div id="tab-design" class="rs-tab-content <?php echo $current_tab === 'design' ? 'active' : ''; ?>">
        <?php
        // DOZO v7.2: Check if design panel exists
        $design_panel_path = RS_WARRANTY_PLUGIN_DIR . 'templates/admin/panels/design/panel-design-settings.html';
        
        if (file_exists($design_panel_path)) {
            // Include the design panel
            include $design_panel_path;
        } else {
            // Show placeholder/message
            ?>
            <div class="rs-settings-card">
                <div style="text-align: center; padding: 60px 20px;">
                    <svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" style="margin: 0 auto 20px; opacity: 0.3;">
                        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" stroke-width="2"/>
                    </svg>
                    <h2 style="margin: 0 0 12px;">🎨 Panel de Diseño</h2>
                    <p style="color: #6b7280; margin: 0 0 24px;">
                        El panel de diseño personalizado no está disponible actualmente.
                    </p>
                    <div style="background: linear-gradient(135deg, rgba(255,140,0,0.1), rgba(255,140,0,0.05)); border: 2px dashed rgba(255,140,0,0.3); border-radius: 12px; padding: 24px; max-width: 600px; margin: 0 auto;">
                        <h3 style="margin: 0 0 12px; color: #FF8C00;">📋 Para activar el panel de diseño:</h3>
                        <ol style="text-align: left; color: #374151; line-height: 1.8; margin: 0; padding-left: 20px;">
                            <li>Asegúrate de tener los archivos de diseño de Claude AI</li>
                            <li>Agrega las etiquetas DOZO:
                                <code style="background: #f3f4f6; padding: 2px 8px; border-radius: 4px; font-size: 12px;">
                                    &lt;!-- @dozo:sync auto --&gt;<br>
                                    &lt;!-- @dozo:panel type="design" --&gt;
                                </code>
                            </li>
                            <li>Ejecuta DOZO Sync desde la consola del navegador</li>
                            <li>El panel aparecerá automáticamente aquí</li>
                        </ol>
                    </div>
                    <div style="margin-top: 24px;">
                        <button type="button" class="rs-btn rs-btn-primary" onclick="dozoExecuteSync()" style="display: inline-flex; align-items: center; gap: 8px;">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                <path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0118.8-4.3M22 12.5a10 10 0 01-18.8 4.2" stroke-width="2"/>
                            </svg>
                            Sincronizar Panel de Diseño
                        </button>
                    </div>
                </div>
            </div>
...
jQuery(document).ready(function($) {
    // DOZO v7.0: Calculate and display health score
    function updateHealthBar() {
        $.ajax({
            url: ajaxurl,
            type: 'POST',
            data: {
                action: 'rs_dozo_get_health',
                nonce: rsWarrantyAdmin.nonce
            },
            success: function(response) {
                if (response.success) {
                    const score = response.data.score || 0;
                    const $progress = $('#dozoHealthProgress');
                    const $score = $('#dozoHealthScore');
                    const $status = $('#dozoHealthStatus');
                    const $message = $('#dozoHealthMessage');
                    ...
                    if (score >= 90) {
                        $progress.css('background', 'linear-gradient(90deg, #10b981, #22c55e)');
                        $score.css('color', '#10b981');
                        $message.text('✅ Sistema en óptimas condiciones');
                    } else if (score >= 70) {
                        $progress.css('background', 'linear-gradient(90deg, #f59e0b, #fbbf24)');
                        $score.css('color', '#f59e0b');
                        $message.text('⚠️ Sistema saludable con advertencias menores');
                    } else {
                        $progress.css('background', 'linear-gradient(90deg, #ef4444, #f87171)');
                        $score.css('color', '#ef4444');
                        $message.text('❌ Se requiere atención - ejecutar limpieza');
                    }
    ...
    // Calculate health on page load
    updateHealthBar();
    
    // DOZO v7.0: Enhanced diagnostic button handler
    $('#runDozoDiagnostic').on('click', function(e) {
        ...
        $output.show().text('🧩 DOZO v7.0 - Iniciando diagnóstico completo...\n');
...
// DOZO v7.2: Execute design panel sync
function dozoExecuteSync() {
    if (!confirm('¿Deseas sincronizar el panel de diseño desde Claude AI?\n\nEsto copiará los archivos de diseño al plugin.')) {
        return;
    }
    ...
    btn.disabled = true;
    btn.innerHTML = '<svg class="rs-spinner" width="20" height="20" viewBox="0 0 24 24" style="animation: spin 1s linear infinite;"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none" opacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10" stroke="currentColor" stroke-width="4" fill="none" stroke-linecap="round"/></svg> Sincronizando...';
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            if (response.success) {
                alert('✅ DOZO Sync completado:\n\n' + 
                    'Archivos sincronizados: ' + response.data.synced_files + '\n' +
                    'Errores: ' + response.data.errors + '\n\n' +
                    'Recargando página...');
                window.location.reload();
            } else {
                alert('❌ Error en sync: ' + (response.data.message || 'Error desconocido'));
                btn.disabled = false;
