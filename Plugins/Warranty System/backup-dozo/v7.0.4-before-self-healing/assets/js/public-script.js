...
function validateStep3() {
    ...
    if (!description.value.trim() || description.value.trim().length < 20) {
        markFieldInvalid(description, 'La descripción debe tener al menos 20 caracteres');
        showNotification('Describe el problema con al menos 20 caracteres', 'error');
        isValid = false;
    } else {
    ...
    if (uploadedFiles.length === 0) {
        showNotification('Debes subir al menos 1 foto o video del defecto', 'error');
        highlightUploadArea();
        isValid = false;
    }
...
function shakeInvalidFields() {
    const invalidFields = document.querySelectorAll('.rs-form-input:invalid, .rs-form-select:invalid, .rs-form-textarea:invalid');
...
function handleFiles(files) {
    Array.from(files).forEach(file => {
        ...
        const currentVideos = uploadedFiles.filter(f => f.type.startsWith('video/')).length;
        ...
        addFileToList(file);
        showNotification(`${isVideo ? 'Video' : 'Imagen'} agregado exitosamente`, 'success');
    });
}
...
function removeFile(filename) {
    ...
    const fileItem = document.querySelector(`.rs-file-item[data-filename="${CSS.escape(filename)}"]`);
    if (fileItem) {
...
function addInputAnimations() {
    const inputs = document.querySelectorAll('.rs-form-input, .rs-form-select, .rs-form-textarea');
