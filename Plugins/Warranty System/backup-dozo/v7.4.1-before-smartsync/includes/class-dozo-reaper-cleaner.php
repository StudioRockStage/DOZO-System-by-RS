 ...
 * MD5 verification, detailed logging, and essential file protection.
 * 
 * @package RockStage_Warranty_System
 * @subpackage DOZO_v7.0.1
 * @since 7.0.0
 * @updated 7.0.1 - Added safe mode, throttling, essential file protection
 */
...
class RS_DOZO_Reaper_Cleaner {
    ...
    private function __construct() {
        ...
        if (defined('DOZO_SAFE_MODE') && DOZO_SAFE_MODE === true) {
            ...
            return; // Disable all Reaper operations in safe mode
        }
    ...
    /**
     * DOZO v7.0.1: Safe mode notice
     */
    public function safe_mode_notice() {
        ?>
        <div class="notice notice-warning is-dismissible">
            <p><strong>🛡️ DOZO Safe Mode Activo:</strong> Limpieza y diagnóstico automático deshabilitados temporalmente. Para desactivar, elimina <code>define('DOZO_SAFE_MODE', true);</code> de wp-config.php</p>
        </div>
        <?php
    }
    ...
    private function rotate_logs_if_needed() {
        ...
        if (filesize($this->log_file) > $max_size) {
            ...
            error_log('DOZO v7.0.1: Log rotated due to size - ' . basename($rotated_name));
        }
    }
    ...
    private function clean_obsolete_files($mode = 'quick') {
        ...
        foreach ($scan['files'] as $file_info) {
            ...
            // DOZO v7.0.1: Check if file is protected (essential)
            if ($this->is_protected_file($file)) {
                $results['protected']++;
                $results['log'][] = 'Protected (essential): ' . basename($file);
                continue;
            }
    ...
    private function is_protected_file($file) {
        ...
        $file_path = str_replace(RS_WARRANTY_PLUGIN_DIR, '', $file);
        
        // Check against protected patterns
        foreach ($this->protected_patterns as $pattern) {
            if (strpos($basename, $pattern) !== false || strpos($file_path, $pattern) !== false) {
                return true;
            }
        }
        ...
        foreach ($protected_dirs as $dir) {
            if (strpos($file_path, $dir) !== false && !preg_match('/\.(bak|old|tmp)$/', $basename)) {
                return true;
            }
        }
