...
(function (window) {
  ...
  DOZO.coreCheck = function () {
    return new Promise((resolve) => {
      ...
      this.checkAjaxOperational().then((ajaxCheck) => {
        ...
        this.log(
          `✅ Core Layer: ${results.score}/${results.maxScore} (${percentage.toFixed(0)}%)`,
          "groupEnd",
        );
  ...
  DOZO.checkNonceUniqueness = function () {
    const nonces = Array.from(
      document.querySelectorAll('input[name*="nonce"]'),
    );
    ...
    return {
      passed,
      message: passed
        ? `${nonces.length} nonces únicos detectados`
        : `${duplicates} nonces duplicados, ${duplicateIds} IDs duplicados`,
      count: nonces.length,
      duplicates,
      duplicateIds,
    };
  ...
  DOZO.checkDynamicCounters = function () {
    ...
    const hasReloadFunction =
      typeof window.rsReloadCategoryStats === "function";
    ...
    return {
      passed,
      message: passed
        ? `Contadores: ${active} activas / ${inactive} inactivas (Total: ${total})`
        : "Contadores en 0/0 o función de recarga no disponible",
      active,
      inactive,
      total,
      hasReloadFunction,
  ...
  DOZO.checkRaceConditionPrevention = function () {
    ...
    const hasDebounced =
      typeof window.rsReloadCategoryTableDebounced === "function";
    ...
    return {
      passed,
      message: passed
        ? `4/4 mecanismos presentes: ${features.join(", ")}`
        : `Faltan mecanismos: ${4 - features.length}`,
  ...
  DOZO.uiCheck = function () {
    return new Promise((resolve) => {
      ...
      this.log(
        `✅ UI Layer: ${results.score}/${results.maxScore} (${percentage.toFixed(0)}%)`,
        "groupEnd",
      );
  ...
  DOZO.checkShortcodeRendering = function () {
    ...
    return {
      passed: hasContent,
      message: hasContent
        ? `Shortcode renderizado: ${type}`
        : "No se detectó contenido del plugin",
      type,
    };
  };
  ...
  DOZO.checkCssLoaded = function () {
    ...
    const warrantyStyles = stylesheets.filter(
      (link) =>
    ...
    return {
      passed,
      message: passed
        ? `${totalStyles} estilos del plugin cargados (${warrantyStyles.length} external, ${inlineStyles.length} inline)`
        : "No se detectaron estilos del plugin",
  ...
  DOZO.checkJsLoaded = function () {
    ...
    const warrantyScripts = scripts.filter(
      (script) =>
    ...
    const passed =
      warrantyScripts.length > 0 || hasRsWarranty || hasRsWarrantyAdmin;

    return {
      passed,
      message: passed
        ? `${warrantyScripts.length} scripts cargados, ${globalObjects.length} objetos globales`
        : "JavaScript del plugin no detectado",
  ...
  DOZO.checkVisualElements = function () {
    ...
    return {
      passed,
      message: passed
        ? `${foundElements.length}/${criticalSelectors.length} elementos críticos encontrados`
        : "No se encontraron elementos visuales del plugin",
      found: foundElements,
  ...
  DOZO.selfHealingCheck = function () {
    return new Promise((resolve) => {
      ...
      this.checkBackendPHP().then((backendCheck) => {
        ...
        this.log(
          `✅ Self-Healing Layer: ${results.score}/${results.maxScore} (${percentage.toFixed(0)}%)`,
          "groupEnd",
        );
  ...
  DOZO.checkBackendPHP = function () {
    return new Promise((resolve) => {
      if (!window.ajaxurl) {
        resolve({
          passed: true,
          message: "Backend validation no aplicable (frontend público)",
        });
        return;
      }
        ...
        success: function (response) {
          ...
          resolve({
            ...
            message: response.success
              ? `Backend PHP operativo (${responseTime}ms)`
              : "Backend responde pero sin success",
            responseTime,
            data: response.data,
          });
        },
  ...
  DOZO.checkCounterFix = function () {
    ...
    const hasCounterElements =
      document.querySelector("#activeCount, #activeCount2") !== null;

    // Verificar que admin-categories.js está cargado
    const hasScript = Array.from(document.querySelectorAll("script[src]")).some(
      (s) => s.src.includes("admin-categories.js"),
    );
    ...
    return {
      passed,
      message: passed
        ? "Fix de contadores presente y funcional"
        : "Fix de contadores faltante o incompleto",
      hasReloadFunction,
      hasCounterElements,
      hasScript,
    };
  ...
  DOZO.checkRaceFix = function () {
    ...
    return {
      passed,
      message: passed
        ? "Prevención de race condition completa (4/4)"
        : `Prevención incompleta (${activeCount}/4)`,
      mechanisms,
      activeCount,
    };
  };
  ...
  DOZO.healCounters = function () {
    return new Promise((resolve) => {
      ...
      if (typeof window.rsReloadCategoryStats !== "function") {
        ...
        script.src =
          (window.rsWarrantyAdmin?.assetsUrl ||
            "/wp-content/plugins/rockstage-warranty-system/assets/") +
          "js/admin-categories.js?t=" +
          Date.now();
        script.onload = () => {
  ...
  DOZO.persistenceCheck = function () {
    return new Promise((resolve) => {
      ...
      this.log(
        `✅ Persistence Layer: ${results.score}/${results.maxScore} (${percentage.toFixed(0)}%)`,
        "groupEnd",
      );
  ...
  DOZO.checkVersionTracking = function () {
    ...
    if (!lastVersion) {
      ...
      return {
        passed: true,
        message: "Primera ejecución de DOZO, version registrada",
    ...
    if (lastVersion !== currentVersion) {
      this.log(
        `⚙️ Actualización detectada: ${lastVersion} → ${currentVersion}`,
      );
      localStorage.setItem("dozo_last_version", currentVersion);
      localStorage.setItem("dozo_last_update", new Date().toISOString());
      return {
        passed: true,
        message: `Actualizado de ${lastVersion} a ${currentVersion}`,
        version: currentVersion,
        updated: true,
      };
    }
  ...
  DOZO.checkHistoricalFixes = function () {
    const fixes = [
      {
        id: "v3.9",
        name: "Nonce IDs únicos",
        check: () => document.querySelectorAll('[id*="nonce"]').length > 0,
      },
      {
        id: "v4.0",
        name: "Race condition prevention",
        check: () => typeof window.rsIsSaving !== "undefined",
      },
      { id: "v4.1", name: "Nonce backend sync", check: () => true }, // Backend, no verificable en JS
      {
        id: "v4.4",
        name: "Claude design import",
        check: () => document.querySelector(".rs-bg-decoration") !== null,
      },
    ];
    ...
    return {
      passed,
      message: passed
        ? `${activeCount}/${results.length} fixes históricos activos`
        : `Solo ${activeCount}/${results.length} fixes activos`,
      fixes: results,
      activeCount,
  ...
  DOZO.checkDegradation = function () {
    ...
    if (history.length < 2) {
      return {
        passed: true,
        message: "Historial insuficiente para detectar degradación",
    ...
    return {
      passed,
      message: passed
        ? `Sin degradación detectada (trend: ${trend.toFixed(1)}%)`
        : `Degradación detectada (trend: ${trend.toFixed(1)}%)`,
      trend,
      avgScore: avgRecent,
      samples: history.length,
    };
  ...
  DOZO.reportIssue = function (moduleName, passed) {
    ...
    if (passed) {
      ...
      if (this.issues[moduleName].successes >= this.config.thresholdStable) {
        ...
        this.log(
          `✅ Módulo "${moduleName}" confirmado estable (${this.issues[moduleName].successes} éxitos consecutivos)`,
        );
      }
    } else {
      ...
      if (this.issues[moduleName].failures >= this.config.thresholdUnstable) {
        ...
        this.warn(
          `⚠️ Módulo "${moduleName}" marcado como INESTABLE (${this.issues[moduleName].failures} fallos consecutivos)`,
        );
        this.requestManualReview(
          moduleName,
          `Módulo inestable tras ${this.issues[moduleName].failures} fallos`,
        );
      }
    }
  };
  ...
  DOZO.requestManualReview = function (moduleName, reason) {
    ...
    const alerts = JSON.parse(
      localStorage.getItem("dozo_manual_reviews") || "[]",
    );
    alerts.push(alert);
  ...
  DOZO.displayResults = function (results) {
    ...
    this.log(
      `${statusEmoji[results.status]} Overall Status: ${results.status.toUpperCase()}`,
    );
    this.log(
      `📊 Score: ${results.score}/${results.maxScore} (${results.percentage.toFixed(1)}%)`,
    );
    ...
    Object.keys(results.layers).forEach((layerName) => {
      ...
      const emoji =
        layerPercent === 100 ? "✅" : layerPercent >= 75 ? "⚠️" : "❌";

      this.log(
        `  ${emoji} ${layerName}: ${layer.score}/${layer.maxScore} (${layerPercent.toFixed(0)}%)`,
      );
    ...
    const unstableModules = Object.keys(this.issues).filter(
      (key) => this.issues[key].status === "unstable",
    );
    if (unstableModules.length > 0) {
      ...
      unstableModules.forEach((mod) => {
        this.warn(
          `   🔴 ${mod}: ${this.issues[mod].failures} fallos consecutivos`,
        );
      });
    }
...
console.log(
  "%c🧩 DOZO v4.8 %cModular Adaptive Diagnostic",
  "color: #FF8C00; font-weight: bold; font-size: 16px;",
...
// ═══════════════════════════════════════════════════════════════
// DOZO v5.4: CLAUDE STYLE AUTO-SYNC & FORCE INJECTION LAYER
// ═══════════════════════════════════════════════════════════════

DOZO.claudeStyleCheck = function () {
  return new Promise((resolve) => {
    this.log("🎨 DOZO Claude Style AutoSync & Force Injection (v5.4)", "group");

    const results = {
      layer: "Claude Style AutoSync",
      version: "5.4",
      passed: false,
      checks: {},
      timestamp: new Date().toISOString(),
    ...
    // Check 1: Public CSS loaded
    this.log("Verificando CSS forzado...");
    ...
    this.log(
      publicCssLoaded
        ...
        : "❌ Claude CSS not found",
      publicCssLoaded ? "success" : "error",
    );

    // Check 2: Public JS loaded
    this.log("Verificando JS forzado...");
    const publicJsLoaded = [...document.scripts].some((script) => {
      try {
    ...
    results.checks.publicJs = {
      name: "Public JS (Claude)",
      passed: publicJsLoaded,
      priority: 999,
    };
    ...
    this.log(
      publicJsLoaded
        ...
        : "❌ Claude JS not found",
      publicJsLoaded ? "success" : "error",
    );

    // Check 3: Force injection marker
    this.log("Verificando marcador de inyección forzada...");
    const forceInjected = [...document.styleSheets].some((sheet) => {
      try {
        ...
        return inlineStyles.includes(
          "DOZO v5.4: Claude Styles Active - Force Injection",
        );
      } catch (e) {
        return false;
      }
    ...
    results.checks.forceInjection = {
      name: "Force Injection Marker",
      passed: forceInjected,
    };

    this.log(
      forceInjected
        ? "✅ Marcador de inyección forzada encontrado"
        : "⚠️ Marcador no encontrado",
      forceInjected ? "success" : "warn",
    );

    // Check 4: No conflicting styles
    this.log("Verificando ausencia de estilos conflictivos...");
    const hasConflicts = [...document.styleSheets].some((sheet) => {
      try {
        return (
          sheet.href &&
          (sheet.href.includes("old-warranty-style") ||
            sheet.href.includes("rs-warranty-style"))
        );
      } catch (e) {
        return false;
      }
    ...
    results.checks.noConflicts = {
      name: "No Conflicting Styles",
      passed: !hasConflicts,
    };
    ...
    this.log(
      !hasConflicts
        ...
        : "⚠️ Estilos conflictivos detectados",
      !hasConflicts ? "success" : "warn",
    );

    // Overall pass/fail
    results.passed = publicCssLoaded && publicJsLoaded;

    this.log("");
    this.log(
      `📊 Resultado: ${results.passed ? "✅ PASSED" : "❌ FAILED"}`,
      results.passed ? "success" : "error",
    );
    this.log(
      "Resumen: " +
        Object.values(results.checks).filter((c) => c.passed).length +
        "/" +
        Object.keys(results.checks).length +
        " checks passed",
    );
...
console.log("");
console.log(
  "🎨 DOZO v5.4: Claude Style AutoSync & Force Injection Layer loaded",
);
console.log("  Run: dozoTestClaudeStyles() to verify Claude styles");
console.log("  Force Injection: Priority 999 (dequeues old styles)");

// ═══════════════════════════════════════════════════════════════
// DOZO v6.1: CLAUDE HTML TEMPLATE INTEGRATION CHECK
// ═══════════════════════════════════════════════════════════════

DOZO.claudeTemplateCheck = function () {
  return new Promise((resolve) => {
    this.log("📄 DOZO Claude HTML Template Integration (v6.1)", "group");

    const results = {
      layer: "Claude HTML Templates",
      version: "6.1",
      passed: false,
      checks: {},
      timestamp: new Date().toISOString(),
    ...
    this.log("Verificando marcadores de plantilla Claude...");
    const hasClaudeMarker =
      document.querySelector('[data-claude-version="6.1"]') !== null;

    results.checks.claudeMarker = {
      name: "Claude Template Marker",
      passed: hasClaudeMarker,
    };

    this.log(
      hasClaudeMarker
        ? "✅ Plantilla Claude detectada (v6.1)"
        : "⚠️ Sin marcador Claude (usando fallback)",
      hasClaudeMarker ? "success" : "warn",
    );

    // Check 2: AJAX variables
    this.log("Verificando variables AJAX...");
    const hasAjaxVars =
      typeof window.rsWarranty !== "undefined" &&
      window.rsWarranty.ajax_url &&
      window.rsWarranty.nonce;

    results.checks.ajaxVars = {
      name: "AJAX Variables Localized",
      passed: hasAjaxVars,
    };
    ...
    this.log(
      hasAjaxVars
        ...
        : "⚠️ Variables AJAX no encontradas",
      hasAjaxVars ? "success" : "warn",
    );

    // Check 3: Shortcode content
    this.log("Verificando contenido de shortcode...");
    const hasShortcodeContent =
      document.querySelector(
        ".rs-claude-template, .warranty-container, .warranty-card",
      ) !== null;

    results.checks.shortcodeContent = {
      name: "Shortcode Content Present",
      passed: hasShortcodeContent,
    };

    this.log(
      hasShortcodeContent
        ? "✅ Contenido de shortcode renderizado"
        : "ℹ️ Sin shortcode en esta página",
      hasShortcodeContent ? "success" : "info",
    );

    // Overall pass/fail
    results.passed = hasClaudeMarker || hasShortcodeContent;

    this.log("");
    this.log(
      `📊 Resultado: ${results.passed ? "✅ PASSED" : "ℹ️ INFO"}`,
      results.passed ? "success" : "info",
    );
    this.log(
      "Resumen: " +
        Object.values(results.checks).filter((c) => c.passed).length +
...
// Expose Claude template verification
window.dozoTestClaudeTemplates = function () {
  return DOZO.claudeTemplateCheck();
};

console.log("");
console.log("📄 DOZO v6.1: Claude HTML Template Integration Layer loaded");
console.log("  Run: dozoTestClaudeTemplates() to verify template integration");
