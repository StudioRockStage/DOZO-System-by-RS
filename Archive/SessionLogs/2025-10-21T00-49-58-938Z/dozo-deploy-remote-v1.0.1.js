...
async function uploadToServer(retryCount = 0) {
  log('☁️', `Conectando al servidor FTP (intento ${retryCount + 1}/${MAX_RETRIES})...`);
  ...
  try {
    ...
    if (retryCount < MAX_RETRIES - 1) {
      ...
      throw new Error(`Error al subir archivo después de ${MAX_RETRIES} intentos: ${error.message}`);
    }
  }
}
...
async function validateDeployment(localSize, localSHA256) {
  ...
  try {
    ...
    const zipResponse = await fetch(DOWNLOAD_URL, { method: 'HEAD', timeout: 10000 });
    ...
    if (validations.zip_accessible) {
      ...
      if (validations.zip_size_match) {
        ...
        log('⚠️', `Diferencia de tamaño: Local ${formatBytes(localSize)} vs Remoto ${formatBytes(remoteSize)}`);
      }
    } else {
...
async function registerDeploy(prepareInfo, uploadInfo, updateData, validationInfo) {
  ...
  const deployReport = {
    ...
    certification: {
      ...
      stability_level: prepareInfo.seal ? prepareInfo.seal.stability_level : 'STABLE'
    },
...
(async () => {
  console.log('\n╔══════════════════════════════════════════════════════════════════════════════╗');
  console.log('║                                                                              ║');
  console.log('║          🚀 DOZO Deploy v1.0.1 – Remote Release (SmartPanel) 🚀             ║');
  console.log('║                                                                              ║');
  console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    // Paso 4: Validación final
    const validationInfo = await validateDeployment(prepareInfo.size, prepareInfo.sha256);
    
    // Paso 5: Registrar deploy
    const deployReport = await registerDeploy(prepareInfo, uploadInfo, updateData, validationInfo);
    ...
    if (validationInfo.allValid) {
      console.log('║                    ✅ DEPLOY EXITOSO COMPLETADO ✅                            ║');
    } else {
      console.log('║               ⚠️  DEPLOY COMPLETADO CON ADVERTENCIAS ⚠️                     ║');
    }
    ...
    console.log('\n╔══════════════════════════════════════════════════════════════════════════════╗');
    console.log('║                                                                              ║');
    console.log('║  ✅ Deploy exitoso: Warranty System RS v1.0.1 ya disponible para             ║');
    console.log('║     actualización automática en WordPress                                    ║');
    console.log('║                                                                              ║');
    console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
    
    // Mostrar estado de validaciones
    console.log('📊 Estado de validaciones:');
    console.log(`   ${validationInfo.validations.zip_accessible ? '✅' : '❌'} ZIP accesible`);
    console.log(`   ${validationInfo.validations.zip_size_match ? '✅' : '❌'} Tamaño coincide`);
    console.log(`   ${validationInfo.validations.json_accessible ? '✅' : '❌'} update.json accesible`);
    console.log(`   ${validationInfo.validations.version_correct ? '✅' : '❌'} Versión correcta`);
    console.log(`   ${validationInfo.validations.download_url_correct ? '✅' : '❌'} URL correcta`);
    console.log(`   ${validationInfo.validations.json_format_correct ? '✅' : '❌'} Formato JSON correcto`);
    
    if (!validationInfo.allValid) {
      console.log('\n⚠️  Algunas validaciones fallaron. Revisa el reporte para detalles.');
      console.log('   Es posible que necesites esperar propagación DNS/CDN o verificar configuración del servidor.\n');
    }
