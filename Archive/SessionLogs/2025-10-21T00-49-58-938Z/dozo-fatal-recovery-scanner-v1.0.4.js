...
const reportPath = path.join(baseDir, 'to chat gpt', 'Global', 'DOZO-FatalScan-Report.json');
...
`;
  ...
  if (!fs.existsSync(mainFile)) {
    console.error('   ❌ No se encontró warranty-system-rs.php, se aborta el escaneo.');
    process.exit(1);
  }
  ...
  mainContent = mainContent.replace(/RS_WARRANTY_VERSION',\s*'1\.0\.3'/, "RS_WARRANTY_VERSION', '1.0.4'");
...
// v1.0.4 - Fatal recovery scan, missing class stubs created`
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      changelog: 'Fatal recovery scan completed. Missing core class files detected and restored with proper stubs.'
    }, null, 2)
  );
  ...
  console.log(`   Estado: ${created.length > 0 ? '✅ REPARADO' : '✅ COMPLETO'}\n`);
  ...
  if (created.length > 0) {
    console.log(`⚠️  NOTA: Se crearon ${created.length} stub(s). Estos deben ser reemplazados con implementaciones completas.\n`);
  } else {
    console.log('🎉 Todos los archivos críticos están presentes. No se requirieron reparaciones.\n');
  }
})();
