...
function getPromptList(dir) {
  ...
  return fs.readdirSync(dir).filter(f => f.endsWith('.js') || f.endsWith('.json') || f.endsWith('.md'));
}
...
function syncVersions() {
  if (!fs.existsSync(versionsFile)) {
    ...
    fs.writeFileSync(versionsFile, JSON.stringify({ version: 'v7.9', prompts: {}, updated: new Date().toISOString() }, null, 2));
  }
