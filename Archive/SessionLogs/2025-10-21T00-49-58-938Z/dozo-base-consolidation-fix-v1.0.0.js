...
function findRealBaseZip() {
  ...
  for (const dir of allSearchDirs) {
    ...
    for (const f of fs.readdirSync(dir)) {
      ...
      if (
        lower.endsWith('.zip') &&
        /warranty.*system.*7\.7\.[0-9]/i.test(lower)
      ) {
  ...
  candidates.sort(
    (a, b) => b.minorVersion - a.minorVersion || b.mtime - a.mtime
  );
...
function patchZip(src, dst) {
  ...
  const phpCandidates = entries
    .filter(e => !e.isDirectory && e.entryName.toLowerCase().endsWith('.php'))
    .map(e => e.entryName)
    .filter(n => /(warranty).*(system)|rockstage-warranty/i.test(n));

  const mainPHP =
    phpCandidates.sort((a, b) => a.length - b.length)[0] || phpCandidates[0];
  if (!mainPHP)
    throw new Error(
      'No se encontró el archivo principal PHP dentro del ZIP para patch.'
    );
...
(async () => {
  ...
  if (!baseZip) {
    const msg =
      '❌ No se encontró un ZIP v7.7.x real (>1MB). Sube el ZIP auténtico y reintenta.';
    console.error(msg);
    fs.writeFileSync(
      REPORT,
  ...
  console.log(`\n📦 ZIP generado: ${dstZip}`);
  console.log(
    `   Tamaño: ${(dstSize / 1024 / 1024).toFixed(2)} MB (${dstSize.toLocaleString()} bytes)`
  );
  ...
  console.log(
    `   Estado: ${sizeOK ? '✅ Válido' : '❌ Inválido (muy pequeño)'}`
  );
