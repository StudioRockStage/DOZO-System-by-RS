...
const VALIDATION_REPORT = path.join(GLOBAL, 'DOZO-CoreVersions-ValidationReport.json');
...
function prepareValidationEnvironment() {
  ...
  if (missing.length > 0) {
    throw new Error(`Archivos faltantes: ${missing.join(', ')}. Detener ejecución.`);
  }
...
function validateDozoCore() {
  ...
  const validations = {
    file_exists: true,
    file_readable: false,
    project_name: { expected: EXPECTED.project_name, actual: null, valid: false },
    active_version: { expected: EXPECTED.active_version, actual: null, valid: false },
    build_status: { expected: EXPECTED.build_status, actual: null, valid: false },
  ...
  try {
    ...
    if (data.warranty_system) {
      ...
      if (data.warranty_system.project_name) {
        ...
        validations.project_name.valid = data.warranty_system.project_name === EXPECTED.project_name;
      }
      
      // Active version
      if (data.warranty_system.version_actual) {
        validations.active_version.actual = data.warranty_system.version_actual;
        validations.active_version.valid = data.warranty_system.version_actual === EXPECTED.active_version;
      }
      
      // Build status
      if (data.warranty_system.estado) {
        validations.build_status.actual = data.warranty_system.estado;
        validations.build_status.valid = data.warranty_system.estado === EXPECTED.build_status;
      }
      
      // Build path
      if (data.warranty_system.build_path) {
        validations.build_path.actual = data.warranty_system.build_path;
        validations.build_path.valid = data.warranty_system.build_path === BUILD_ZIP;
      }
    }
    
    // Log results
    Object.keys(validations).forEach(key => {
      if (typeof validations[key] === 'object' && validations[key].hasOwnProperty('valid')) {
        if (validations[key].valid) {
          log('✅', `${key}: ${validations[key].actual}`);
        } else {
          log('⚠️', `${key}: esperado "${validations[key].expected}", encontrado "${validations[key].actual}"`);
        }
      }
    });
    
    const allValid = Object.keys(validations)
      .filter(key => typeof validations[key] === 'object' && validations[key].hasOwnProperty('valid'))
...
function validateVersions() {
  ...
  const validations = {
    file_exists: true,
    file_readable: false,
    version_actual: { expected: EXPECTED.active_version, actual: null, valid: false },
    build_path: { expected: BUILD_ZIP, actual: null, valid: false },
  ...
  try {
    ...
    if (data.version_actual) {
      ...
      validations.version_actual.valid = data.version_actual === EXPECTED.active_version;
    }
...
function generateValidationReport(coreValidation, versionsValidation) {
  ...
  const buildSHA256 = versionsValidation.validations.sha256.actual || calculateSHA256(BUILD_ZIP);
  
  const report = {
    action: 'DOZO Core & Versions Validation',
    status: coreValidation.allValid && versionsValidation.allValid ? 'success' : 'partial_success',
  ...
  Object.keys(coreValidation.validations).forEach(key => {
    if (typeof coreValidation.validations[key] === 'object' && 
  ...
  Object.keys(versionsValidation.validations).forEach(key => {
    if (typeof versionsValidation.validations[key] === 'object' && 
  ...
  if (!coreValidation.allValid) {
    report.recommendations.push('Revisar y corregir manualmente DOZO-Core.json');
  }
  
  if (versionsValidation.validations.sha256.needs_update) {
    report.recommendations.push('SHA256 fue actualizado automáticamente en Versions.json');
  }
...
(async () => {
  console.log('\n╔══════════════════════════════════════════════════════════════════════════════╗');
  console.log('║                                                                              ║');
  console.log('║          🔍 DOZO Validation – Core & Versions Integrity Check 🔍             ║');
  console.log('║                                                                              ║');
  console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    if (coreValidation.allValid && versionsValidation.allValid) {
      ...
      console.log('║            ⚠️  VALIDACIÓN COMPLETADA CON CORRECCIONES ⚠️                    ║');
    }
    
    console.log('║                                                                              ║');
    console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
    
    log('📊', `Validaciones: ${report.summary.passed} pasadas, ${report.summary.failed} fallidas, ${report.summary.corrected} corregidas`);
