...
const extractedDir = path.join(
  latestBuilds,
  "Warranty_System_RS_v1.0.1_extracted",
);
const reportPath = path.join(
  baseDir,
  "to chat gpt",
  "Global",
  "DOZO-Congruence-Audit.json",
);
...
function validateHeader(content, filePath) {
  ...
  expected.forEach((field) => {
    ...
    if (!match || match[1].trim() !== field.value) {
      ...
      console.log(
        `   ❌ ${field.key}: esperado "${field.value}", encontrado "${match ? match[1].trim() : "undefined"}"`,
      );
    } else {
      validations.push({
        type: "header_valid",
...
function validateConstants(content, filePath) {
  ...
  constants.forEach((constant) => {
    ...
    if (!match) {
      ...
      console.log(
        `   ❌ ${constant.name}: esperado "${constant.expectedValue}", encontrado "${match[1]}"`,
      );
    } else {
      validations.push({
        type: "constant_valid",
  ...
  const expectedDirs = [
    {
      path: "includes",
      required: true,
      description: "Clases y funciones core",
    },
    ...
    { path: "assets", required: true, description: "CSS, JS y recursos" },
    {
      path: "Admin Panels",
      required: false,
      description: "Paneles de administración",
    },
  ...
  expectedDirs.forEach((dir) => {
    ...
    if (!fs.existsSync(dirPath)) {
      if (dir.required) {
        ...
        console.log(
          `   ❌ Directorio faltante (requerido): ${dir.path} - ${dir.description}`,
        );
      } else {
        warnings.push({
          type: "missing_optional_directory",
          dir: dir.path,
          description: dir.description,
        });
...
function validateAdminPanel(baseDir) {
  ...
  const adminFiles = [
    {
      path: "includes/class-warranty-admin.php",
      description: "Core Admin Class",
    },
    ...
    {
      path: "templates/admin/create-warranty.php",
      description: "Create Warranty Form",
    },
...
function validateNaming(baseDir) {
  ...
  files.forEach((file) => {
    ...
    if (file.isFile() && file.name.endsWith(".php")) {
      ...
      if (
        !file.name.match(/^(warranty|class-|index|functions|template)/i) &&
  ...
  if (namingIssues === 0) {
    ...
    validations.push({
      type: "naming_valid",
      message: "Todos los archivos siguen convenciones",
    });
  } else {
    console.log(
      `   ⚠️  ${namingIssues} archivos con nomenclatura no estándar (ver reporte)`,
    );
  }
}
...
function validateVersionConsistency() {
  ...
  const files = [
    ...
    {
      path: path.join(baseDir, "updates", "warranty-system", "update.json"),
      key: "version",
    },
  ];
    ...
    if (fs.existsSync(file.path)) {
      try {
        ...
        if (version === "1.0.1") {
          ...
          console.log(
            `   ❌ ${path.basename(file.path)}: esperado v1.0.1, encontrado v${version}`,
          );
        }
      } catch (err) {
        warnings.push({
        ...
        console.log(
          `   ⚠️  Error al leer ${path.basename(file.path)}: ${err.message}`,
        );
      }
    }
  });
...
(async () => {
  ...
  if (
    !checkFileExists(mainFile, "Archivo principal (warranty-system-rs.php)")
  ) {
    issues.push({
      type: "critical",
      message: "Archivo principal no encontrado: warranty-system-rs.php",
    });
  } else {
    ...
    summary: {
      ...
      congruence_score:
        validations.length > 0
          ? (
              (validations.length / (validations.length + issues.length)) *
              100
            ).toFixed(1) + "%"
          : "0%",
    },
