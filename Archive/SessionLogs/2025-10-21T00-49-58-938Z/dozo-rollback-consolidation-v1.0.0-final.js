...
const globalLog = path.join(
  ...
  "to chat gpt",
  "Global",
  "DOZO-RollbackConsolidation-Report.json",
);
...
const finalZip = path.join(
  latestBuilds,
  "Warranty_System_RS_v1.0.0_STABLE.zip",
);
...
const toDelete = fs
  ...
  .filter(
    (f) => f.match(/Warranty_System_RS_v1\.0\.[0-9]/) && !f.includes("STABLE"),
  );
...
if (toDelete.length === 0) {
  console.log(
    "   ℹ️  No se encontraron versiones experimentales para eliminar",
  );
}
  ...
  JSON.stringify(
    {
      ...
      changelog:
        "Stable base version consolidated from real plugin source. Complete functionality, admin panel, and WordPress compatibility verified.",
    },
    null,
    2,
  ),
...
console.log(
  "🎉 Warranty System RS v1.0.0 STABLE - Base funcional consolidada!\n",
);
console.log(
  "💡 Esta es la versión base estable con toda la funcionalidad real del plugin.",
);
console.log(
  "🚀 Lista para instalación, testing y actualizaciones automáticas.\n",
);
