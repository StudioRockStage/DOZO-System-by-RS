...
const sourceZip = path.join(latestBuilds, 'Warranty_System_v7.5.5_20251015_174919.zip');
...
(async () => {
  ...
  if (!fs.existsSync(sourceZip)) {
    console.error('   ❌ No se encontró el archivo base Warranty_System_v7.5.5_20251015_174919.zip');
  ...
  const toClean = fs.readdirSync(latestBuilds).filter(file => 
    file.match(/Warranty_System_RS_v1\.0\.[0-9]/) && file.includes('STABLE')
  );
  ...
  const oldReports = fs.readdirSync(globalDir).filter(f => 
    f.match(/DOZO.*Report\.json/) && !f.includes('RebaseUpgrade')
  );
  ...
  if (!fs.existsSync(mainFile)) {
    ...
    const phpFiles = fs.readdirSync(pluginDir).filter(f => f.endsWith('.php'));
  ...
  if (!fs.existsSync(mainFile)) {
    ...
    console.log('   📂 Archivos en directorio:', fs.readdirSync(pluginDir).slice(0, 10));
    process.exit(1);
  }
...
`;
  ...
  if (mainFile !== newMainFile && path.basename(mainFile) !== 'warranty-system-rs.php') {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      changelog: 'Stable base version v1.0.0 consolidated from functional v7.5.5. Complete features, DOZO auto-update integration, and WordPress compatibility verified.'
    }, null, 2)
  );
  ...
  console.log('🎉 Warranty System RS v1.0.0 - Base estable desde v7.5.5 funcional!\n');
