...
for (const c of candidates) {
  if (
    fs.existsSync(c) &&
    fs.existsSync(path.join(c, 'warranty-system-rs.php'))
  ) {
    srcDir = c;
    break;
  }
...
if (!srcDir) {
  throw new Error(
    'No se encontró la carpeta fuente del plugin con warranty-system-rs.php. Verifica que exista alguna de las rutas candidatas.'
  );
}
...
for (const d of requiredDirs) {
  if (!fs.existsSync(path.join(srcDir, d))) {
    report.warnings.push(`Falta directorio requerido: ${d}`);
  }
}
for (const f of requiredFiles) {
  if (!fs.existsSync(path.join(srcDir, f))) {
    report.warnings.push(`Falta archivo requerido: ${f}`);
  }
}
...
['claude/shortcodes', 'claude/designs'].forEach(rel => {
...
if (!entriesZip.length || !entriesZip[0].startsWith('warranty-system-rs/')) {
  throw new Error(
    'El ZIP no tiene la carpeta raíz correcta (warranty-system-rs/).'
  );
}
...
const sessionDumpDir = path.join(
  archiveDir,
  new Date().toISOString().replace(/[:.]/g, '-')
);
ensureDir(sessionDumpDir);
const workspaceLoose = fs
  .readdirSync(baseDir)
  .filter(
    n =>
      /\.(md|txt|log|js)$/i.test(n) &&
      !['package.json', 'package-lock.json'].includes(n)
  );
for (const n of workspaceLoose) {
  fs.renameSync(path.join(baseDir, n), path.join(sessionDumpDir, n));
}
report.actions.push({ archived_loose_files: workspaceLoose });

// Limpiar `to chat gpt` (mover todo a papelera interna)
const gptTrash = path.join(baseDir, 'Archive', 'Trash');
ensureDir(gptTrash);
if (fs.existsSync(toChatGPT)) {
  const items = fs.readdirSync(toChatGPT).filter(n => n !== 'Global');
  for (const n of items) {
    fs.renameSync(
      path.join(toChatGPT, n),
      path.join(gptTrash, `${Date.now()}_${n}`)
    );
  }
