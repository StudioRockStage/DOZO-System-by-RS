...
// Archivos fuente
const BASE_ZIP = path.join(ROOT, 'Latest Builds', 'Warranty_System_v7.5.5_20251015_174919.zip');
const SMART_PANEL_HTML = path.join(ROOT, 'Claude AI', 'DISEÑOS Warranty System RS', 'SmartCategoryPanel_Approved_DOZO_v1.1.0.html');
  ...
  dirsToScan.forEach(dir => {
    if (fs.existsSync(dir)) {
      ...
      items.forEach(item => {
        ...
        if (item.endsWith('.zip') && (item.includes('with') || item.includes('rev') || item.includes('test'))) {
...
async function rebuildV100() {
  ...
  const dirs = fs.readdirSync(workV100).filter(d => 
    fs.statSync(path.join(workV100, d)).isDirectory()
  );
  ...
  const oldMainFiles = fg.sync(['rockstage-warranty-system.php', 'warranty-*.php'], { cwd: sourcePath });
  const oldMainPath = oldMainFiles.length > 0 ? path.join(sourcePath, oldMainFiles[0]) : null;
  const newMainPath = path.join(sourcePath, 'warranty-system-rs.php');
  
  if (oldMainPath && oldMainPath !== newMainPath) {
    log('📝', `Renombrando: ${path.basename(oldMainPath)} → warranty-system-rs.php`);
 ...
 */`;
  ...
  content = content.replace(/define\s*\(\s*'RS_WARRANTY_VERSION'\s*,\s*'[^']+'/g, "define('RS_WARRANTY_VERSION', '1.0.0'");
  content = content.replace(/define\s*\(\s*'RS_DOZO_VERSION'\s*,\s*'[^']+'/g, "define('RS_DOZO_VERSION', '1.0.0'");
  ...
  const filesV100 = fg.sync(['**/*'], { cwd: finalV100Dir, dot: false, onlyFiles: true });
...
async function buildV101(v100Info) {
  ...
  content = content.replace(/@version\s+1\.0\.0/g, '@version 1.0.1');
  content = content.replace(/define\s*\(\s*'RS_WARRANTY_VERSION'\s*,\s*'1\.0\.0'/g, "define('RS_WARRANTY_VERSION', '1.0.1'");
  content = content.replace(/define\s*\(\s*'RS_DOZO_VERSION'\s*,\s*'1\.0\.0'/g, "define('RS_DOZO_VERSION', '1.0.1'");
...
`;
    ...
    fs.writeFileSync(path.join(adminDir, 'smart-category-panel.php'), phpWrapper);
    fs.writeFileSync(path.join(publicDir, 'smart-category-panel.php'), phpWrapper);
...
if ( file_exists( RS_WARRANTY_PLUGIN_DIR . 'admin/smart-category-panel.php' ) ) {
    ...
    if (insertPoint > -1) {
      mainContent = mainContent.slice(0, insertPoint) + includeCode + mainContent.slice(insertPoint);
  ...
  const filesV101 = fg.sync(['**/*'], { cwd: workV101, dot: false, onlyFiles: true });
...
function cleanupOldVersions() {
  ...
  if (cleaned.files.length + cleaned.directories.length === 0) {
    ...
    log('✅', `Limpieza completada: ${cleaned.files.length} archivos, ${cleaned.directories.length} directorios`);
  }
...
function validateConsistency(v100Info, v101Info) {
  ...
  try {
    ...
    const hasMainFile = entries100.some(e => e.entryName.includes('warranty-system-rs.php'));
    const hasCorrectStructure = entries100.some(e => e.entryName.startsWith('warranty-system-rs/'));
    ...
    if (hasCorrectStructure) log('✅', 'v1.0.0: Estructura de carpeta correcta');
  } catch (e) {
  ...
  try {
    ...
    const hasSmartPanel = entries101.some(e => e.entryName.includes('smart-category-panel.php'));
...
function generateDualBuildReport(audit, v100Info, v101Info, cleaned, validation) {
  ...
  const report = {
    ...
    dozo_version: '7.9',
    audit_summary: {
      issues_found: audit.folders_with_version_suffix.length + audit.incorrect_php_files.length,
...
(async () => {
  console.log('\n╔══════════════════════════════════════════════════════════════════════════════╗');
  console.log('║                                                                              ║');
  console.log('║              🧩 DOZO Dual Build Reconstructor v1.0 🧩                        ║');
  console.log('║                                                                              ║');
  console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    // Paso 6: Reporte
    const report = generateDualBuildReport(audit, v100Info, v101Info, cleaned, validation);
    ...
    console.log('\n╔══════════════════════════════════════════════════════════════════════════════╗');
    console.log('║                                                                              ║');
    console.log('║                  ✅ DUAL BUILD COMPLETADO EXITOSAMENTE ✅                     ║');
    console.log('║                                                                              ║');
    console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
