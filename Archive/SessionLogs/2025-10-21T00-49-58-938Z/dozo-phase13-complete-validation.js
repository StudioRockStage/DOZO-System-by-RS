...
const URLS = {
  ...
  zipV777: 'https://updates.vapedot.mx/warranty-system/Warranty_System_v7.7.7.zip',
  zipV776: 'https://updates.vapedot.mx/warranty-system/Warranty_System_v7.7.6.zip',
};
...
async function checkHTTP(url, name) {
  try {
    ...
    return { accessible, status: res.status, headers: Object.fromEntries(res.headers.entries()) };
  } catch (err) {
...
try {
  ...
  validation.checks.update_json_content = { accessible: false, error: err.message };
}
...
try {
  const ftpConfig = JSON.parse(fs.readFileSync(path.join(SCRIPTS, 'ftp-config.json'), 'utf8'));
  ...
  validation.checks.ftp_verification = { accessible: false, error: err.message };
}
...
validation.summary = {
  update_json_accessible: validation.checks.http_accessibility?.updateJson?.accessible || false,
  version_on_cdn: httpVersion,
  version_on_server: ftpVersion,
  cache_status: validation.checks.cache_comparison?.status || 'unknown',
  files_on_server_verified: validation.checks.ftp_verification?.accessible || false,
  ready_for_wordpress: ftpVersion === '7.7.7' && validation.checks.ftp_verification?.has_v777
};

console.log(`update.json HTTP:     ${validation.summary.update_json_accessible ? '✅' : '❌'}`);
...
console.log(`Archivos verificados: ${validation.summary.files_on_server_verified ? '✅' : '❌'}`);
console.log(`Listo para WP:        ${validation.summary.ready_for_wordpress ? '✅' : '⏳'}`);

validation.status = 'completed';
validation.overall_status = validation.summary.ready_for_wordpress ? 
  'ready_pending_cache' : 'files_ready_cache_pending';
...
if (validation.summary.cache_status === 'stale') {
  ...
  console.log('   Espera 5-10 minutos y ejecuta de nuevo: npm run validate:live');
