...
const REPORT = path.join(ROOT, 'to chat gpt', 'Global', 'DOZO-BaseSurgery-Report.json');
...
].map(n => path.join(LATEST, n)).filter(p => fs.existsSync(p));

if (!candidates.length) {
  console.error('❌ No se encontró ZIP base en Latest Builds/Warranty System RS/');
  process.exit(1);
}
...
if (path.basename(PLUGIN_DIR) !== 'warranty-system-rs') {
  ...
  console.log(`🧭 Renombrando ${path.basename(PLUGIN_DIR)} → warranty-system-rs/`);
...
if (!fs.existsSync(MAIN_PHP)) {
  console.log('⚠️  warranty-system-rs.php no encontrado, buscando alternativa...');
  // Busca un main alterno y renómbralo
  const candidatesPhp = fs.readdirSync(path.join(TMP, 'warranty-system-rs'))
    .filter(f => f.endsWith('.php'));
  const fallback = candidatesPhp.find(f => /warranty|rockstage|main|plugin/i.test(f));
  if (!fallback) {
    console.error('❌ No se encontró archivo PHP principal del plugin.');
    process.exit(1);
  }
  moveSync(
    path.join(TMP, 'warranty-system-rs', fallback),
    MAIN_PHP,
    { overwrite: true }
  );
...
php = php
  .replace(/^<\?php\s*\/\*\*[\s\S]*?\*\/\s*/m, (hdr) => {
    // Reescribe manteniendo descripción si la hubiera
    const desc = /Description:\s*(.*)/i.test(hdr) ? RegExp.$1.trim() :
...
const ADMIN_CLASS = path.join(TMP, 'warranty-system-rs', 'includes', 'class-warranty-admin.php');
...
const report = {
  ...
  fixed: {
    ...
    headers: ['Plugin Name','Version','Text Domain','Update URI','Requires','Requires PHP'],
    basename: 'RS_WARRANTY_PLUGIN_BASENAME = plugin_basename(__FILE__)',
    settings_link: true,
    admin_menu_minimum: (!fs.existsSync(ADMIN_DIR) || !fs.existsSync(ADMIN_CLASS)) ? 'injected' : 'kept',
