...
// Archivos
const SOURCE_ZIP = path.join(EMPAQUETADO_READY, 'warranty-system-rs-v1.0.1.zip');
...
function prepareEnvironment() {
  ...
  directories.forEach(dir => {
    if (!fs.existsSync(dir)) {
      ...
      log('📁', `Directorio creado: ${path.basename(path.dirname(dir))}/${path.basename(dir)}`);
      created++;
    }
  });
...
function moveBuildToLatest() {
  ...
  if (fs.existsSync(LATEST_BUILDS)) {
    ...
    const oldVersions = files.filter(f => 
  ...
  if (fs.existsSync(VERSIONS_JSON)) {
    try {
      ...
      if (data.version_actual === '1.0.1') {
        ...
        log('❌', `Versions.json tiene versión incorrecta: ${data.version_actual}`);
      }
    } catch (e) {
...
(async () => {
  console.log('\n╔══════════════════════════════════════════════════════════════════════════════╗');
  console.log('║                                                                              ║');
  console.log('║           📦 DOZO Build Relocation & Core Update v1.0.1 📦                   ║');
  console.log('║                                                                              ║');
  console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    console.log('\n╔══════════════════════════════════════════════════════════════════════════════╗');
    console.log('║                                                                              ║');
    console.log('║                  ✅ BUILD CONSOLIDADO CORRECTAMENTE ✅                        ║');
    console.log('║                                                                              ║');
    console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
