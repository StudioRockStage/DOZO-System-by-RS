...
let ftp = null;
if (WANT_DEPLOY) {
  try { ftp = (await import('basic-ftp')).default; } catch {}
}
...
function loadFTPConfig(){
  ...
  if (fs.existsSync(FTP_JSON)) Object.assign(out, JSON.parse(fs.readFileSync(FTP_JSON,'utf8')));
  return {
    ...
    remotePath: out.FTP_PATH || out.remotePath || '/public_html/updates/warranty-system/'
  };
}
...
function findLatestZip(){
  ...
  const zips = fs.readdirSync(READY).filter(f => /Warranty_System_v\d+\.\d+\.\d+\.zip$/i.test(f));
  if(!zips.length) throw new Error('No hay ZIP en Ready/');
  const sorted = zips.map(f=>({f,t:fs.statSync(path.join(READY,f)).mtimeMs})).sort((a,b)=>b.t-a.t);
...
async function maybeDeploy(zipName){
  ...
  if(!cfg.host||!cfg.user||!cfg.password) throw new Error('Credenciales FTP incompletas');
  const client = new ftp.Client(30000);
  try{
    await client.access({host:cfg.host,user:cfg.user,password:cfg.password,port:cfg.port,secure:cfg.secure});
    const startDir = await client.pwd();
    // normalizar ruta
    let remote = cfg.remotePath;
    if (startDir.includes('/public_html') && remote.startsWith('/public_html/')) remote = remote.replace('/public_html','');
    if(!remote.startsWith('/')) remote = '/' + remote;
    if(!remote.endsWith('/')) remote += '/';

    await client.ensureDir(remote); await client.cd(remote);
...
(async () => {
  ...
  ensureDir(LATEST_DIR); ensureDir(GLOBAL_DIR);
  ...
  for(const f of fs.readdirSync(LATEST_DIR)) fs.rmSync(path.join(LATEST_DIR,f), {force:true, recursive:false});
  fs.copyFileSync(path.join(READY, latestZip), path.join(LATEST_DIR, latestZip));
  ...
  const updateJSON = {
    ...
    tested: '6.6.2', requires: '5.9', requires_php: '7.4',
    last_updated: new Date().toISOString().slice(0,10),
    sections: { description: 'Sistema de gestión post‑compra (garantías, tickets, etc.)', changelog: `v${version} — Build sincronizado (Phase 12).` }
  };
  writeJSON(UPDATE_JSON_LOCAL, updateJSON);

  // 3) Metadatos DOZO-LATEST.json
  const latestMeta = { plugin: PKG, slug: SLUG, version, filename: latestZip, readyPath: path.join(READY, latestZip), latestBuildPath: path.join(LATEST_DIR, latestZip), download_url, updatedAt: new Date().toISOString() };
  writeJSON(LATEST_META, latestMeta);

  // 4) Reporte global
  writeJSON(REPORT, { phase: 12, status: 'ok', actions: ['sync-latest-build','write-update.json','write-metadata'], latestMeta });

  // 5) Deploy opcional
  const deployRes = await maybeDeploy(latestZip);
  if (deployRes.deployed) {
    const rep = JSON.parse(fs.readFileSync(REPORT,'utf8')); rep.deploy = deployRes; writeJSON(REPORT, rep);
