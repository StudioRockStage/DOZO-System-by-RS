...
const writeReport = (obj) => {
  ...
  fs.writeFileSync(REPORT, JSON.stringify({ ts: new Date().toISOString(), ...obj }, null, 2));
};
...
const phpLint = (dir) => {
  ...
  walk(dir);
  const errors = [];
  for (const file of files.slice(0, 50)) { // Limitar a 50 archivos para no tardar demasiado
    try {
      execSync(`php -l "${file}"`, { stdio: 'pipe' });
    } catch (e) {
      errors.push({ file: path.relative(dir, file), message: e.stderr?.toString() || e.message });
    }
  }
  return errors;
};
...
const rewritePluginHeader = (phpPath) => {
  ...
  const set = (label, value) => {
    if (src.match(new RegExp(`^\\s*\\*\\s*${label}:`, 'mi'))) {
      src = src.replace(new RegExp(`(^\\s*\\*\\s*${label}:).*$`, 'mi'), `$1 ${value}`);
    } else {
...
(async () => {
  ...
  try {
    ...
    if (!fs.existsSync(BASE_ZIP)) {
      ...
      writeReport({ ok: false, step: 'precheck', error: 'ZIP base no encontrado', expected: BASE_ZIP, latestDir: LATEST, latestFiles: list });
    ...
    const top = fs.readdirSync(TMP).filter(f => fs.statSync(path.join(TMP, f)).isDirectory());
    ...
    const phpCandidates = fs.readdirSync(pluginDir).filter(f => f.endsWith('.php'));
    if (phpCandidates.length === 0) throw new Error('No se encontró archivo PHP en la raíz del plugin.');
    ...
    if (lintErrors.length) {
      console.log(`   ⚠️  ${lintErrors.length} advertencias de sintaxis detectadas`);
      console.log('   ℹ️  Continuando con el build (modo de advertencia)');
      steps.push({ step: 'php-lint', status: 'warnings', errors: lintErrors.slice(0, 10) });
    ...
    steps.push({ step: 'cleanup-ready', kept: path.basename(OUT_ZIP), cleaned: cleanedCount });
    console.log();
    ...
    console.log('\n═══════════════════════════════════════════════════════════════════════════════════════\n');
    console.log('🎉 Warranty System RS v1.0.0 - Build quirúrgico completado!\n');
