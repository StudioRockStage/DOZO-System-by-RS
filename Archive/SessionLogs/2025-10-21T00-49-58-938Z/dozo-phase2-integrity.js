...
const reportPath = path.join(basePath, 'to chat gpt', 'Global', 'DOZO-Integrity-Report.json');
...
function validateFolders() {
  ...
  const missing = requiredFolders.filter(f => !fs.existsSync(path.join(basePath, f)));
...
function validateFiles() {
  ...
  const missing = requiredFiles.filter(f => !fs.existsSync(path.join(workflowPath, f)));
  return missing.length ? { ok: false, missing } : { ok: true };
}
