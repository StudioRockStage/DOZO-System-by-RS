...
['Empaquetado', 'Plugins/Warranty System', 'Backup', 'Latest Builds'].forEach(
  folder => {
    ...
    if (fs.existsSync(dir)) {
      ...
      files.forEach(file => {
        if (
          file.match(/warranty|rockstage|RS/i) &&
          !file.match(/\.dozo_lock/)
        ) {
...
console.log(
  '📦 Copiando y renombrando build v7.7.9 como Warranty System RS v1.0.0...'
);
const oldBuild = path.join(
  baseDir,
  'Empaquetado',
  'Warranty_System_v7.7.9.zip'
);
...
fs.writeFileSync(
  ...
  JSON.stringify(
    { active_plugin: pluginName, version, certified_base: true },
    null,
    2
  )
);
...
const report = {
  status: 'success',
  message:
    'Warranty System RS v1.0.0 consolidado exitosamente como versión base.',
  plugin: pluginName,
  author: pluginAuthor,
  version,
...
console.log(
  '🚀 Warranty System RS v1.0.0 listo para desarrollo y actualizaciones automáticas.'
);
