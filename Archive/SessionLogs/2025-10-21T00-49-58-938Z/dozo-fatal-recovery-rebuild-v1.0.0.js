...
const BASE_ZIP = path.join(LATEST, 'Warranty_System_v7.5.5_20251015_174919.zip');
...
function log(obj){ fs.appendFileSync(LOG, JSON.stringify({ts:new Date().toISOString(), ...obj})+'\n'); }
function ensureDirs(){ [LATEST_UPDATES, GLOBAL, WORK_TMP].forEach(d=>{ if(!fs.existsSync(d)) fs.mkdirSync(d,{recursive:true});}); }
function cleanAll(){ [LATEST_UPDATES, WORK_TMP].forEach(p=>{ if(fs.existsSync(p)) fs.rmSync(p,{recursive:true,force:true});}); ensureDirs(); }

function unzip(zipFile, dest){ const z = new AdmZip(zipFile); z.extractAllTo(dest, true); }
...
function replaceHeaders(file){
  ...
  s = s.replace(/define\('RS_WARRANTY_VERSION',\s*'[\d\.]+'/g, "define('RS_WARRANTY_VERSION', '1.0.0'");
  s = s.replace(/define\('RS_DOZO_VERSION',\s*'[\d\.]+'/g, "define('RS_DOZO_VERSION', '1.0.0'");
  fs.writeFileSync(file,s);
}

function validateStructure(dir){
  const required=['rockstage-warranty-system.php','includes/class-warranty-core.php','includes/class-warranty-admin.php'];
...
function deepAnalysis(dir){
  ...
  analysis.structure.admin = fs.existsSync(path.join(dir,'Admin Panels')) || fs.existsSync(path.join(dir,'admin'));
  ...
  if(fs.existsSync(mainFile)){
    ...
    // Find hooks
    const actionMatches = content.matchAll(/add_action\s*\(\s*['"]([^'"]+)['"]/g);
    for(const match of actionMatches) analysis.hooks.actions.push(match[1]);
    
    const filterMatches = content.matchAll(/add_filter\s*\(\s*['"]([^'"]+)['"]/g);
    ...
    analysis.dependencies.hpos = /HPOS|woocommerce_feature_custom_order_tables/.test(content);
  }
...
(async()=>{
  ...
  const dirs = fs.readdirSync(WORK_TMP).filter(d => fs.statSync(path.join(WORK_TMP,d)).isDirectory());
  ...
  if(missing.length){ 
    ensureDirs();
    fs.appendFileSync(LOG, JSON.stringify({ts:new Date().toISOString(),phase:'validation',missing})+'\n');
  ...
  console.log(`   • WooCommerce: ${analysisFinal.dependencies.woocommerce ? '✓' : '✗'}`);
