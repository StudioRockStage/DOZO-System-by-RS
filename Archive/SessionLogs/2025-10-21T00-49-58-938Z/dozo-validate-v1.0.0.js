...
const reportPath = path.join(baseDir, "to chat gpt", "Global", "DOZO-Validation-Report.json");
...
function checkJSON(filePath, description, expectedKeys) {
  ...
  try {
    if (!fs.existsSync(filePath)) {
      ...
      checks.push({ check: description, path: filePath, status: "❌ FAIL", error: "File not found" });
      return false;
    }
    
    const content = JSON.parse(fs.readFileSync(filePath, "utf8"));
    const hasKeys = expectedKeys.every(key => content.hasOwnProperty(key) || JSON.stringify(content).includes(key));
    
    if (hasKeys) {
      passedChecks++;
      console.log(`✅ ${description}`);
      checks.push({ check: description, path: filePath, status: "✅ PASS", content });
      return true;
    } else {
      failedChecks++;
      console.log(`❌ ${description} - Claves faltantes`);
      checks.push({ check: description, path: filePath, status: "❌ FAIL", error: "Missing keys", content });
      return false;
    }
  } catch (error) {
    failedChecks++;
    console.log(`❌ ${description} - Error: ${error.message}`);
    checks.push({ check: description, path: filePath, status: "❌ FAIL", error: error.message });
    return false;
  }
}
...
checkFile(
  path.join(baseDir, "to chat gpt", "Global", "DOZO-BaseConsolidation-Report.json"),
...
const phpPath = path.join(baseDir, "Plugins", "Warranty System", "warranty-system-rs.php");
if (fs.existsSync(phpPath)) {
  const phpContent = fs.readFileSync(phpPath, "utf8");
  const phpChecks = [
    { pattern: /Plugin Name:\s*Warranty System RS/, desc: "Plugin Name correcto" },
    ...
    { pattern: /RS_WARRANTY_PLUGIN_NAME/, desc: "Constante RS_WARRANTY_PLUGIN_NAME" },
...
const legacyChecks = [
  { pattern: /7\.[0-9]+\.[0-9]+/, dir: "Empaquetado", desc: "Sin versiones 7.x.x en Empaquetado" },
  { pattern: /7\.[0-9]+\.[0-9]+/, dir: "Latest Builds", desc: "Sin versiones 7.x.x en Latest Builds" },
  { pattern: /rockstage-warranty-system\.php/, dir: "Plugins/Warranty System", desc: "Sin archivo legacy rockstage-warranty-system.php" }
];
...
legacyChecks.forEach(({ pattern, dir, desc }) => {
  ...
  if (fs.existsSync(dirPath)) {
    ...
    if (!hasLegacy) {
      ...
      checks.push({ check: desc, status: "❌ FAIL", type: "Legacy Check", found: files.filter(f => pattern.test(f)) });
    }
  }
});
...
console.log(`📈 Tasa de Éxito: ${((passedChecks / totalChecks) * 100).toFixed(1)}%`);
