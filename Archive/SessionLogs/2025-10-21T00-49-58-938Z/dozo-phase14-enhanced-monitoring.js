...
const scriptFiles = fs
  .readdirSync(BASE)
  .filter((f) => f.startsWith("dozo-") && f.endsWith(".js"));
...
if (fs.existsSync(packageJsonPath)) {
  ...
  console.log(
    `   ✅ Dependencies: ${Object.keys(npmDeps.dependencies).length}`,
  );
}
...
const deploymentReports = reportFiles.filter(
  (f) =>
    f.includes("Deploy") ||
...
const deployments = deploymentReports
  .map((file) => {
    const filePath = path.join(GLOBAL_DIR, file);
    const stats = fs.statSync(filePath);
    return {
...
const monitoringData = {
  ...
  inventory: {
    scripts: {
      ...
      total_size_kb: scripts
        .reduce((sum, s) => sum + parseFloat(s.size_kb), 0)
        .toFixed(2),
      list: scripts,
    },
    documentation: {
      ...
      total_size_kb: documentation
        .reduce((sum, d) => sum + parseFloat(d.size_kb), 0)
        .toFixed(2),
      list: documentation,
    },
    reports: {
      count: reports.length,
      total_size_kb: reports
        .reduce((sum, r) => sum + parseFloat(r.size_kb), 0)
        .toFixed(2),
      list: reports,
    },
    packages: {
      count: packages.length,
      total_size_mb: packages
        .reduce((sum, p) => sum + parseFloat(p.size_mb), 0)
        .toFixed(2),
      list: packages,
    },
  },
  ...
  recommendations: [
    ...
    ftpStatus.configured
      ? "✅ FTP properly configured"
      : "⚠️ FTP needs configuration",
...
console.log("═══════════════════════════════════════════════════════════════");
console.log(
  `Total archivos monitoreados: ${monitoringData.statistics.total_files}`,
);
