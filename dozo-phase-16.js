...
const syncMetadata = {
  ...
  releases: {
    available: fs.existsSync(path.join(root, "PublicRelease")) ? 
      fs.readdirSync(path.join(root, "PublicRelease")).filter(f => f.endsWith('.dmg')).length : 0
  },
  phases: {
    completed: 16,
    reports: []
...
console.log(`   Listo para AutoSync: ${report.summary.readyForAutoSync ? '✅ Sí' : '⚠️ Requiere configuración'}`);
console.log("");
