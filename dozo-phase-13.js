...
try {
  const ebVersion = execSync("npx electron-builder --version", { encoding: "utf8" }).trim();
  ...
  try {
    ...
    buildReport.errors.push("Error instalando electron-builder: " + err.message);
  }
}
console.log("");
...
try {
  ...
  console.warn(`   ⚠️  No se pudieron limpiar builds anteriores: ${err.message}`);
...
if (dmgGenerated && dmgPath) {
  try {
    ...
    if (certs.includes("Developer ID Application")) {
      ...
      try {
        execSync(`codesign --sign "Developer ID Application" --deep --force --verbose "${dmgPath}"`, {
          stdio: "inherit"
        });
...
if (dmgGenerated && dmgPath) {
  try {
    const hashOutput = execSync(`shasum -a 256 "${dmgPath}"`, { encoding: "utf8" });
