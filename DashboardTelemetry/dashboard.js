...
function updateSystemMetrics(sys) {
  ...
  if (sys.memUsagePercent > 80) {
    memFill.style.background =
      "linear-gradient(90deg, #f44336 0%, #ff5722 100%)";
  } else if (sys.memUsagePercent > 60) {
    memFill.style.background =
      "linear-gradient(90deg, #ffc107 0%, #ffeb3b 100%)";
  } else {
    memFill.style.background =
      "linear-gradient(90deg, #E6C185 0%, #f5d9a0 100%)";
  }
...
function updateIntegrity(integrity) {
  ...
  if (integrity.directories) {
    const dirsFormatted = Object.entries(integrity.directories)
      .map(([dir, info]) => {
        const status =
          info.status === "OK" ? "✅" : info.status === "MISSING" ? "❌" : "⚠️";
