...
app.get("/api/metrics", (req, res) => {
  ...
  if (!fs.existsSync(reportDir)) {
    return res.json({ 
      error: "Sin reportes de telemetría aún. Ejecuta primero 'node dozo-phase-11.js'" 
    });
  }
  ...
  const sys = {
    ...
    memUsagePercent: (((os.totalmem() - os.freemem()) / os.totalmem()) * 100).toFixed(1),
...
app.get("/api/reports", (req, res) => {
  ...
  const allReports = fs.readdirSync(reportDir)
