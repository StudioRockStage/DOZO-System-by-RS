...
try {
  const certsOutput = execSync("security find-identity -v -p codesigning", { encoding: "utf8" });
...
if (certificateAvailable) {
  try {
    ...
    report.steps.push("DMG firmado digitalmente con " + (certificateName || "Developer ID Application"));
    
    // Verificar la firma
    try {
      const verifyOutput = execSync(`codesign -dv --verbose=4 "${dmgPath}" 2>&1`, { encoding: "utf8" });
